...
class CulturalProgressExtractor {
  ...
  static extractChantedShlokas(gameProgress) {
    ...
    try {
      ...
      // ✅ DEFINE: Shlokas taught in each Shloka River scene (4 scenes, 8 lines total)
      // Inside CulturalProgressExtractor.js -> extractChantedShlokas()

const shlokaVerses = {
  // ✅ SCENE 1: Vakratunda Grove
  'vakratunda-chant': { 
    name: 'Vakratunda Chant', 
    line: 'Vakra-tunda',
    scene: 'vakratunda-grove',
    type: 'word1'
  },
  'mahakaya-chant': { 
    name: 'Mahakaya Chant', 
  ...
  'suryakoti-chant': { 
    name: 'Suryakoti Chant', 
    line: 'Surya-koti',
    scene: 'suryakoti-bank',
    type: 'word1'
  },
  'samaprabha-chant': { 
    name: 'Samaprabha Chant', 
    line: 'Sama-prabha',
    scene: 'suryakoti-bank',
    type: 'word2'
  },
  
  // ✅ SCENE 3: Nirvighnam Cave
  'nirvighnam-chant': { 
    name: 'Nirvighnam Chant', 
    line: 'Nirvighnam',
    scene: 'nirvighnam-kurumedeva', // Check your exact sceneId
    type: 'word1'
  },
  'kurumedeva-chant': { 
    name: 'Kurume Deva Chant', 
    line: 'Kuru-me Deva',
    scene: 'nirvighnam-kurumedeva', // Check your exact sceneId
    type: 'word2'
  },
  
  // ✅ SCENE 4: Sarvakaryeshu Mountain
  'sarvakaryeshu-chant': { 
    name: 'Sarvakaryeshu Chant', 
    line: 'Sarva-karyeshu',
    scene: 'sarvakaryeshu-sarvada', // Check your exact sceneId
    type: 'word1'
  },
  'sarvada-chant': { 
    name: 'Sarvada Chant', 
    ...
    type: 'word2'
      ...
      console.log('🎵 CHANTS EXTRACTION: Starting shloka verse extraction from Shloka River');
        ...
        Object.keys(localStorage).forEach(key => {
          if (key.includes(activeProfileId) && 
            ...
            try {
              ...
              // Check for chanted verses/lines
              if (sceneState.chantedVerses) {
                Object.entries(sceneState.chantedVerses).forEach(([verseId, chanted]) => {
                  if (chanted && shlokaVerses[verseId] && 
                      !chants.find(c => c.verseId === verseId)) {
                    chants.push({
                      verseId: verseId,
                      name: shlokaVerses[verseId].name,
                      line: shlokaVerses[verseId].line,
                      scene: shlokaVerses[verseId].scene,
                      type: shlokaVerses[verseId].type,
                      chantedAt: Date.now(),
                      source: 'current-session'
                    });
                    console.log(`✅ CURRENT SESSION: ${verseId} from temp session`);
                  }
                });
              }
            } catch (e) {
            ...
            if (scene.completed && scene.chantedVerses) {
              Object.entries(scene.chantedVerses).forEach(([verseId, chanted]) => {
                if (chanted && shlokaVerses[verseId] && 
                  ...
                  console.log(`✅ COMPLETED SHLOKA: ${verseId} from ${sceneId}`);
                }
              });
            }
          });
      ...
      console.log(`🎵 CHANTS TOTAL: ${chants.length} shloka verses chanted`, 
                  chants.map(c => `${c.verseId} (${c.source})`));
