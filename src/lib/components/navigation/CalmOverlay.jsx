...
export default function CalmOverlay({ sym, onClose, onDone }) {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => { setCycleCount(1); setBreathState('inhale'); }, 820);
  ...
  // ── Dismiss handler (backdrop tap OR ✕ click) ────────────────────────────
  const handleDismiss = () => {
    if (exiting) return;
    setExiting(true);
    setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      {/* Ganesha silhouette watermark */}
      <div className="co-watermark" aria-hidden="true">🐘</div>

      {/* ── Panel — click stops propagation so tap-inside ≠ dismiss ─────── */}
      <div
        className={[
          'co-panel',
      ...
      >
        {/* Soft close — top right, 40% opacity */}
        <button className="co-close" onClick={handleDismiss} aria-label="Close">✕</button>

        {/* 1. Symbol micro icon — 28px, 70% opacity */}
        <span className="co-symbol-icon" aria-hidden="true">{sym.emoji || '🐘'}</span>

        {/* 2. Technique label — uppercase eyebrow */}
        <p className="co-technique-label">{(sym.technique || 'Breathing Together').toUpperCase()}</p>

        {/* 3. Story text */}
        <p className="co-story-text">{sym.resetStory}</p>

        {/* 4. Breathing ripple area */}
        ...
        {/* 5. Breath cue — key triggers fade-in on every phase change */}
        {breathCue && (
          <p className="co-breath-cue" key={breathState}>{breathCue}</p>
        )}

        {/* 6. Closure line — fades in after all 3 cycles */}
