...
const SunCollectionGame = ({ 
  ...
  // ✅ AUTO-START for scene integration
  useEffect(() => {
    if (autoStart) {
      console.log('🌞 Auto-starting sun collection for scene integration');
      setGameActive(true);
    }
  }, [autoStart]);

  // ✅ SHARE ORB POSITION for Game 2 integration - MOVED TO SIDE
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // ✅ NEW: Clear all floating objects immediately
    setFloatingObjects([]);
    console.log('🧹 Cleared all floating objects on completion');
