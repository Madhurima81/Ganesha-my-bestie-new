...
const SimpleFogClicker = ({ emotionId, fogImage, rockImage, onComplete, isActive }) => {
  ...
  const handleClick = () => {
    ...
    console.log(`🔮 Fog clicked: ${emotionId}, count: ${newCount}/3`);
  ...
  if (!isActive && clickCount === 0) return null;

  const opacity = Math.max(0, 1 - (clickCount / 3));
  ...
  return (
    <div 
      ...
      style={{
        ...
        border: isActive && !isCompleted ? '3px solid #FFD700' : '3px solid transparent',
        borderRadius: '12px',
        boxShadow: isActive && !isCompleted ? '0 0 20px rgba(255,215,0,0.5)' : 'none',
    ...
    >
      ...
      {/* Click Instructions */}
      {isActive && !isCompleted && clickCount === 0 && (
        <div style={{
          position: 'absolute',
          top: '10px',
          left: '50%',
...
// ✅ NEW: Simple click-to-place rock component
const SimpleRockPlacer = ({ rockId, rockImage, emotion, onPlace, isPlaced, isUsed }) => {
    ...
    >
      <img 
        ...
        style={{ 
          ...
          height: '100%',
          borderRadius: '12px',
          border: isAnimating ? '3px solid rgba(255, 215, 0, 0.3)' : '3px solid rgba(255, 215, 0, 0.6)',
          boxShadow: isAnimating ? 'none' : '0 4px 8px rgba(0,0,0,0.3)',
          transition: 'all 0.3s ease'
        }}
      />
...
const handleBridgeRockClick = (rockId) => {
  ...
  // 🛡️ PROTECTION LAYERS
  if (now - lastClickTime.current < 300) {  // ✅ Reduced to 300ms for faster sequential clicks
  ...
  // ✅ KEY FIX: Check if THIS SPECIFIC rock is already placed
  if (sceneState.bridgeRocks?.includes(rockId)) {
    console.log('🚫 BLOCKED: Rock already placed');
    return;
  }
  
  // ✅ KEY FIX: Check if bridge is already complete
  if (sceneState.bridgeRocks?.length >= 3) {
    console.log('🚫 BLOCKED: Bridge already complete');
    return;
  }
  
  // ✅ APPROVED - Process click
  console.log(`🌉 Bridge rock clicked: ${rockId}`);
  lastClickTime.current = now;  // ✅ Only 300ms cooldown between ANY rock clicks
...
{!showPowerModal && !showRescueModal && !showCenteredSymbol && !showStoryModal && (
  <>
    ...
    {sceneState.phase === CAVE_PHASES.CRYSTAL_FOG_ACTIVE && sceneState.clearedFogs?.length < 3 && (
      <div className="phase-header">
    🔮 CLEAR THE FOG! Click each crystal 3 times!
      </div>
    )}
