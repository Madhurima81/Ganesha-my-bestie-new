...
const SimpleRockPlacer = ({ rockId, rockImage, emotion, onPlace, isPlaced, isUsed, isClickable = true }) => {  // ✅ ADD isClickable prop
  const [isAnimating, setIsAnimating] = useState(false);
  
  const handleClick = () => {
    // ✅ NEW: Check if clickable first
    if (!isClickable) {
...
return (
    <div 
      ...
      style={{
        width: '80px',
        height: '80px',
        cursor: !isClickable ? 'not-allowed' : (isAnimating ? 'default' : 'pointer'),  // ✅ Show not-allowed cursor
        opacity: !isClickable ? 0.4 : (isAnimating ? 0.3 : 1),  // ✅ Dim non-clickable rocks
...
border: isAnimating ? '3px solid rgba(255, 215, 0, 0.3)' : 
                !isClickable ? '3px solid rgba(128, 128, 128, 0.3)' :  // ✅ Gray border for locked
                '3px solid rgba(255, 215, 0, 0.6)',          boxShadow: isAnimating ? 'none' : '0 4px 8px rgba(0,0,0,0.3)',
          transition: 'all 0.3s ease'
        }}
      />
      
      {!isAnimating && isClickable && (  // ✅ Only show hint if clickable
        <div style={{
          position: 'absolute',
          top: '-30px',
          left: '50%',
...
<div className="bridge-rocks-inventory">
  <div className="inventory-rocks">
    {sceneState.collectedRocks?.map((rockId, index) => {
      ...
      // ✅ NEW: Only current rock is clickable
      ...
      return (
        <div key={rockId} className={`bridge-rock-inventory bridge-rock-${rockId}`}>
          <SimpleRockPlacer
            ...
            isUsed={isUsed}
            isClickable={isClickable}  // ✅ NEW PROP
          />
        </div>
      );
    })}
