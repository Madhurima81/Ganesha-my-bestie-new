      ...
      <SceneManager
        ...
        initialState={{
          ...
          learnedWords: {
           ...
           kurume: { learned: false, scene: 3 },      // ✅ SEPARATE: kurume
  deva: { learned: false, scene: 3 }         // ✅ SEPARATE: deva
          },
          ...
          // ✅ ADD: Text animation flags at root level
  showNirvighnamText: false,
  showKurumedevaText: false,
...
const handleDoor1Complete = () => {
 ...
 const handleCrystalFogComplete = () => {
  ...
  safeSetTimeout(() => {
    console.log('🚨 CALLING completeNirvighnamLearning NOW!'); // ← ADD THIS
    completeNirvighnamLearning(); // ← IS THIS BEING CALLED?
  }, 1000);
};
...
const completeNirvighnamLearning = () => {
  ...
  // ✅ ADD: Browser alert to confirm function runs
  alert('🚨 Nirvighnam text should show NOW!');
  ...
  sceneActions.updateState({
    ...
    showNirvighnamText: true
  });

  // ✅ LONGER TIMING: 8 seconds instead of 3
  setTimeout(() => {
    sceneActions.updateState({ showNirvighnamText: false });
    alert('🚨 Nirvighnam text should HIDE now!'); // ✅ ADD: Second alert
  }, 8000);

  // ✅ LONGER TIMING: 9 seconds instead of 3.5
  setTimeout(() => {
    setShowSparkle(null);
    sceneActions.updateState({
      phase: CAVE_PHASES.DOOR2_ACTIVE
...
const startKurumedevaLearning = () => {
  ...
  console.log('🔍 DEBUG: Current learned words before update:', sceneState.learnedWords);
  
  // Show brief sparkle effect
  setShowSparkle('kurumedeva-to-sidebar');
  
  // ✅ UPDATE: Both kurume AND deva separately
  sceneActions.updateState({
    learnedWords: {
      ...sceneState.learnedWords,
      kurume: { learned: true, scene: 3 },    // ✅ SEPARATE: kurume
      deva: { learned: true, scene: 3 }       // ✅ SEPARATE: deva
    },
    phase: CAVE_PHASES.KURUME_DEVA_LEARNING,
    showKurumedevaText: true  // ✅ Show text!
  });

  console.log('✅ DEBUG: Kurume Deva text should show now');

  // Hide text after animation
  setTimeout(() => {
    sceneActions.updateState({ showKurumedevaText: false });
    console.log('🔍 DEBUG: Kurume Deva text hidden');
  }, 2500);
  ...
  setTimeout(() => {
    setShowSparkle(null);
    showFinalCelebration();
    console.log('🎊 DEBUG: Starting final celebration');
  }, 2000);
};
...
// ❌ COMMENT OUT: handleCloseCard function  
/*
...
<div style={{
  ...
  {/* ✅ Door 1 Done - Should ONLY complete door and show crystal game (NO COMPLETION) */}
<button onClick={() => {
  sceneActions.updateState({
    phase: CAVE_PHASES.CRYSTAL_FOG_INTRO,  // ✅ FIX: Start crystal fog, not complete
    door1Completed: true,
    door1CurrentStep: 3,
    door1SyllablesPlaced: ['Nir', 'vigh', 'nam'],
    crystalFogStarted: false,        // ✅ FIX: Game hasn't started yet
    crystalFogCompleted: false,      // ✅ FIX: Not completed yet
    selectedCrystal: null,
    clearedFogs: [],                 // ✅ FIX: Empty - no fogs cleared yet
    collectedRocks: [],              // ✅ FIX: Empty - no rocks collected yet
    emotionalProgress: 0             // ✅ FIX: No progress yet
  });
  // ✅ FIX: DON'T call completeNirvighnamLearning() here!
}} style={{
...
{/* ✅ Game 1 Done - Should show completion and text */}
<button onClick={() => {
  sceneActions.updateState({
    phase: CAVE_PHASES.CRYSTAL_FOG_COMPLETE,  // ✅ NOW this shows completion
    door1Completed: true,
    crystalFogCompleted: true,
  ...
  // ✅ NOW this calls the learning function
  setTimeout(() => {
  ...
  {/* ✅ Door 2 Done - Should show Kurume Deva learning */}
  <button onClick={() => {
    sceneActions.updateState({
      phase: CAVE_PHASES.BRIDGE_BUILDING_COMPLETE,  // ✅ CHANGED: Go to complete state
      door1Completed: true,
      door2Completed: true,
      door2CurrentStep: 5,
      ...
      bridgeCompleted: true,  // ✅ ADD: Mark as completed
      bridgeRocks: ['anger', 'confusion', 'fear', 'lonely', 'sad'],  // ✅ ADD: All rocks used
      bridgeStability: 100,  // ✅ ADD: Full stability
      ganeshaCanCross: true,  // ✅ ADD: Bridge ready
      // ✅ ADD: Nirvighnam should already be learned
      learnedWords: {
        ...sceneState.learnedWords,
        nirvighnam: { learned: true, scene: 3 }
      }
    ...
    // ✅ ADD: Trigger Kurume Deva learning after state update
    setTimeout(() => {
      startKurumedevaLearning();
    }, 500);
  }} style={{
  ...
  {/* 🎉 Complete Scene - Should show final celebration with all words learned */}
  <button onClick={() => {
    // ✅ FIRST: Set all completion states properly
    sceneActions.updateState({
      phase: CAVE_PHASES.COMPLETE,
      door1Completed: true,
      door2Completed: true,
      ...
      bridgeCompleted: true,
      clearedFogs: ['anger', 'confusion', 'fear', 'lonely', 'sad'],
      collectedRocks: [],  // ✅ Empty because all used in bridge
      bridgeRocks: ['anger', 'confusion', 'fear', 'lonely', 'sad'],
      ganeshaCanCross: true,
      // ✅ BOTH words should be learned
      learnedWords: {
        ...sceneState.learnedWords,
        nirvighnam: { learned: true, scene: 3 },
        kurume: { learned: true, scene: 3 },      // ✅ SEPARATE
        deva: { learned: true, scene: 3 }         // ✅ SEPARATE
      },
      stars: 8,
      completed: true,
      progress: {
    ...
    // ✅ THEN: Trigger final celebration
    setTimeout(() => {
      showFinalCelebration();
    }, 500);
...
{/* 🧪 ENHANCED GAME 1 TEXT TEST */}
<button onClick={() => {
  console.log('🧪 TESTING GAME 1 TEXT - Enhanced version');
  ...
  setTimeout(() => {
    console.log('🎯 Setting showNirvighnamText to TRUE');
    sceneActions.updateState({
      showNirvighnamText: true,
      phase: CAVE_PHASES.NIRVIGHNAM_LEARNING
    });
    ...
    setTimeout(() => {
      console.log('🔍 Current state check:', {
        showNirvighnamText: sceneState.showNirvighnamText,
        phase: sceneState.phase
      });
    }, 100);
  ...
  setTimeout(() => {
    console.log('🎯 Hiding text after 3 seconds');
    sceneActions.updateState({ showNirvighnamText: false });
  }, 3500);
...
<button onClick={() => {
  console.log('🧪 CALLING completeNirvighnamLearning DIRECTLY');
  completeNirvighnamLearning();
}} style={{
  position: 'fixed',
  top: '320px',
...
{/* ✨ NEW: Kurumedeva to Sidebar Effect */}
...
{/* ✅ ENHANCED: Nirvighnam Text with better visibility */}
{sceneState.showNirvighnamText && (
  <div className="nirvighnam-text" style={{
    position: 'fixed',  // ← CHANGED: fixed instead of absolute
    top: '50%',         // ← CHANGED: More centered
    left: '50%',        // ← CHANGED: More centered
    transform: 'translate(-50%, -50%)', // ← CHANGED: Perfect centering
    fontSize: '32px',   // ← CHANGED: Bigger
    fontWeight: 'bold',
    color: '#4ECDC4',
    textShadow: '3px 3px 6px rgba(0,0,0,0.8)', // ← CHANGED: Stronger shadow
    animation: 'textFloat 3s ease-out forwards',
    zIndex: 9999,       // ← CHANGED: Much higher z-index
    background: 'rgba(0,0,0,0.7)', // ← ADDED: Dark background
    padding: '20px 40px', // ← ADDED: Padding
    borderRadius: '15px', // ← ADDED: Rounded corners
    border: '3px solid #4ECDC4' // ← ADDED: Border
  }}>
    Without Obstacles
  </div>
)}

{/* ✅ ENHANCED: Kurumedeva Text with better visibility */}
{sceneState.showKurumedevaText && (
  <div className="kurumedeva-text" style={{
    position: 'fixed',  // ← CHANGED: fixed instead of absolute
    top: '50%',         // ← CHANGED: More centered
    left: '50%',        // ← CHANGED: More centered
    transform: 'translate(-50%, -50%)', // ← CHANGED: Perfect centering
    ...
    padding: '20px 40px', // ← ADDED: Padding
    borderRadius: '15px', // ← ADDED: Rounded corners
    border: '3px solid #FFD700' // ← ADDED: Border
  }}>
    Divine Help
  </div>
)}
...
`}</style>
  ...
  sanskritWords: { nirvighnam: true, kurume: true, deva: true }, // ✅ SEPARATE
  learnedWords: sceneState.learnedWords || {},
  chants: { nirvighnam: true, kurume: true, deva: true },        // ✅ SEPARATE
                completed: true,
                totalStars: 8
              }}
              onComplete={onComplete}
