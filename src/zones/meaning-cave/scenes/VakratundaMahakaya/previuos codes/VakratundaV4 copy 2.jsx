...
// ✅ NEW: Sequential path validation (prevents cheating)
const getSequentialProgress = (x, y) => {
console.log('🔍 Sequential check called:', x, y);
  console.log('🔍 Current segment:', sceneState.currentPathSegment);

  const currentSegment = sceneState.currentPathSegment || 0;
  const targetPoint = pathPoints[currentSegment];

    console.log('🔍 Target point:', targetPoint);
  
  if (!targetPoint) return { progress: -1, quality: 'off-path' };
  
  const distance = Math.sqrt(
    Math.pow(x - targetPoint.x, 2) + Math.pow(y - targetPoint.y, 2)
  );
  
  const tolerance = 45; // Must be within 45px of current target
  
  if (distance < tolerance) {
    // ✅ Reached current segment target
    return {
      progress: targetPoint.progress,
      quality: distance < 25 ? 'good' : 'okay',
      segmentReached: currentSegment
    };
  }
  
  return { progress: -1, quality: 'off-path' };
};
...
onMouseMove={(e) => {
  if (isTracing) {
    ...
    if (coords && coords.x > 0 && coords.y > 0) {
      ...
      const traceResult = getProgressFromCurvedPath(coords.x, coords.y);
