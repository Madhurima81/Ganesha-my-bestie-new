...
const [currentPosition, setCurrentPosition] = useState({ x: 100, y: 150 }); // Start at new starting point  
...
const getGaneshaAnimationClass = () => {
  ...
  const pathPoints = [
  { x: 100, y: 150, progress: 0 },    // Left bottom start
  { x: 200, y: 120, progress: 25 },
  { x: 300, y: 100, progress: 50 },
  { x: 400, y: 90, progress: 75 },
  { x: 480, y: 85, progress: 100 }    // Center dark area end
];
...
const getProgressFromPosition = (x, y) => {
  ...
  // ✅ MUCH STRICTER: Must be very close to path
  const tolerance = 35; // Tighter tolerance
  
  if (closestDistance < tolerance) {
    return {
      progress: bestProgress,
      distance: closestDistance,
      quality: getTraceQuality(closestDistance)
    };
  }
  
  // ✅ CLEARLY OFF PATH
  return { progress: -1, distance: closestDistance, quality: 'off-path' };
};

// ✅ STRICTER: Updated quality thresholds
const getTraceQuality = (distance) => {
  if (distance < 15) return 'perfect';   // Very close to path
  if (distance < 25) return 'good';      // Close enough  
  if (distance < 30) return 'okay';      // Just acceptable
  return 'off-path';                     // Too far
};
...
const getTraceLineColor = (quality, opacity = 1) => {
  ...
  switch(quality) {
    case 'perfect': return `rgba(0, 255, 0${alpha})`;     // Bright green
    case 'good': return `rgba(255, 255, 0${alpha})`;      // Yellow  
    case 'okay': return `rgba(255, 165, 0${alpha})`;      // Orange
    case 'off-path': return `rgba(255, 68, 68${alpha})`;  // Red
    default: return `rgba(255, 215, 0${alpha})`;          // Gold default
  }
};
...
const distanceToRedCircle = Math.sqrt(
  Math.pow(coords.x - 480, 2) + Math.pow(coords.y - 85, 2)
);
...
onMouseDown={(e) => {
  ...
  if (!isTracing) {
    ...
    const distanceFromStart = Math.sqrt(
      Math.pow(coords.x - 100, 2) + Math.pow(coords.y - 150, 2)
    );
    ...
    if (distanceFromStart < 25) {
      ...
      setIsTracing(true);
      setShowCursor(true);
      setCurrentPosition({ x: 100, y: 150 });
      setTracedPath([{ x: 100, y: 150 }]); // Only green circle
...
onMouseMove={(e) => {
  if (isTracing) {
    ...
    if (coords && coords.x > 0 && coords.y > 0) {
      ...
      // ✅ ALWAYS update trace quality for visual feedback
const traceResult = getProgressFromPosition(coords.x, coords.y);
sceneActions.updateState({ 
  traceQuality: traceResult.quality 
});
      
      // ✅ ORIGINAL: Always add to traced path (like before)
      setTracedPath(prev => [...prev, coords]);
            
      // ✅ ORIGINAL: Check progress without path restrictions
      if (traceResult.progress > sceneState.traceProgress) {
        ...
        // Clear off-path messages when making progress
        setShowOffPathText(false);
        setOffPathMessage('');
        
        // ✅ ORIGINAL: Simple completion check
        if (traceResult.progress >= 90) {
          console.log('🎉 Tracing completed!');
          completeTracing();
        }
      } else if (traceResult.progress === -1) {
        // Show guidance when off-path, but still draw line
...
sceneActions.updateState({ 
  traceQuality: traceResult.quality 
      ...
      // ✅ ORIGINAL: Always add to traced path
      setTracedPath(prev => [...prev, coords]);
...
>
            ...
            <path
    d="M 100 150 Q 250 110 350 95 Q 420 88 480 85"  // ← NEW PATH (same as main path)
              stroke="#ff8c42"
              strokeWidth="25"
              fill="none"
              strokeLinecap="round"
...
<path
  d="M 100 150 Q 250 110 350 95 Q 420 88 480 85"
...
<circle cx="480" cy="85" r="15" fill="#ff4444" stroke="#fff" strokeWidth="3">
...
{sceneState.tracingStarted && sceneState.traceQuality && (
  <div style={{
    ...
    {sceneState.traceQuality === 'perfect' && '🌟 Perfect!'}
    {sceneState.traceQuality === 'good' && '✅ Good!'}
    {sceneState.traceQuality === 'okay' && '⚠️ Follow the curve'}
    {sceneState.traceQuality === 'off-path' && '❌ Stay on the orange path!'}
  </div>
)}
