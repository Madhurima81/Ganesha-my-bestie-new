// ClickDotsPathGame.jsx - Integrated Component for Cave Scene
...
const ClickDotsPathGame = ({
  ...
  // Positioning props
  progressBarStyle = {},  // Custom styles for progress bar
  gameAreaStyle = {},     // Custom styles for game area
  showProgressBar = true  // Toggle progress bar visibility
}) => {
  const svgRef = useRef(null);
  const [currentDot, setCurrentDot] = useState(0);
  const [completedDots, setCompletedDots] = useState([]);
  const [mooshikaPosition, setMooshikaPosition] = useState({ x: 120, y: 140 });
  const [isComplete, setIsComplete] = useState(false);
  const [feedbackMessage, setFeedbackMessage] = useState('Click dot 1 to start!');
  ...
  const pathDots = [
    ...
    { x: 460, y: 135, label: '8' },
    { x: 430, y: 110, label: '9' },
    { x: 380, y: 90, label: '10' }  // Game ends here!
  ];

  const showFeedback = (message, duration = null) => {
    setFeedbackMessage(message);
    // Don't auto-hide - message stays visible unless explicitly cleared
    if (duration) {
  ...
  const handleDotClick = (index) => {
    ...
    // Check if clicking the correct next dot
    if (index === currentDot + 1) {
      ...
      // Wrong dot - shake it (auto-hide this error message)
  ...
  const reset = () => {
    ...
    setIsComplete(false);
    setFeedbackMessage('Click dot 1 to start!');  // This message stays visible
  ...
  const svgToContainerCoords = (svgX, svgY) => {
    ...
    return {
      x: (svgX / viewBox.width) * 100,  // Convert to percentage
  ...
  return (
    <div style={{ 
      ...
      <div style={{ position: 'relative', height: '250px' }}>
        ...
        >
          {/* Background guide path (faded) - ends at dot 10 */}
          <path
          ...
          {pathDots.map((dot, index) => {
            ...
            const isEnd = index === pathDots.length - 1;  // Dot 10 is the end
            ...
            if (isStart) {
              ...
              // Dot 10 - make it special (red until reached, gold when complete)
              fillColor = isComplete ? '#FFD700' : '#FF4444';
              strokeColor = isComplete ? '#FFA500' : '#CC0000';
              radius = 25;
              showGlow = isNext;  // Glow when it's the next dot to click
            } else if (isCompleted) {
              fillColor = '#00FF88';
              strokeColor = '#00CC66';
            } else if (isNext) {
        ...
        {mooshikaImage && (
          <div style={{
            position: 'absolute',
            left: `${svgToContainerCoords(mooshikaPosition.x, mooshikaPosition.y).x}%`,
            top: `${svgToContainerCoords(mooshikaPosition.x, mooshikaPosition.y).y}%`,
            transform: 'translate(-50%, -50%)',
            width: '60px',
            height: '60px',
            zIndex: 10,
            pointerEvents: 'none',
            transition: completedDots.length > 0 ? 'left 0.6s ease-out, top 0.6s ease-out' : 'none',
            animation: completedDots.length > 0 ? 'mooshikaHop 0.6s ease-out' : 'none'
          }}>
            <img
              src={mooshikaImage}
              alt="Mooshika"
        ...
        {/* Completion Sparkles - Around Dot 10 */}
        {showSparkles && (
          <div style={{
            position: 'absolute',
            top: '0',
