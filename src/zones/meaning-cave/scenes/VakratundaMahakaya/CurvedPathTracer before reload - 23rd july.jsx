// CurvedPathTracer.jsx - FIXED: SVG to Container Coordinate Conversion
...
const CurvedPathTracer = ({
  ...
  const [mooshikaRotation, setMooshikaRotation] = useState(0); // ✨ ADD THIS LINE
  ...
  // Mooshika rotation calculation - ADD THIS ENTIRE BLOCK
// Mooshika rotation calculation - FIXED
useEffect(() => {
  if (tracedPath.length > 3 && isTracing) {  // ✨ CHANGED: More points + only when tracing
    ...
    if (Math.abs(deltaX) > 3 || Math.abs(deltaY) > 3) {  // ✨ ADDED: Movement threshold
const angle = Math.atan2(-deltaY, deltaX) * (180 / Math.PI);
setMooshikaRotation(angle);      setMooshikaRotation(angle);
    }
  }
}, [tracedPath, isTracing]);
  ...
  const reset = () => {
    ...
    setFeedbackMessage('');
    setMooshikaTrail([]);
      setMooshikaRotation(0);  // ✨ ADD THIS LINE - Reset rotation to 0
