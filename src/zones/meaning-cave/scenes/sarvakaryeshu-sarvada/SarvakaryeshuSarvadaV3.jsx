...
const handleSymbolDrop = (dragData) => {
  ...
  if (isWise) {
    ...
    // Clear message and complete scenario after delay
    setTimeout(() => {
      sceneActions.updateState({
        characterMessage: null,
        characterMessageType: null
      });
      setTimeout(() => completeCurrentScenario(), 1000);
    }, 2500);
  ...
  // ✅ FIXED: Complete current scenario - check for 3 total
...
// ❌ COMMENT OUT THIS - This was showing the success message as GameCoach popup
  /*
  if (showMessage) {
    showMessage(scenario.successMessage, {
      duration: 3000,
  ...
  */
    ...
    if (scenariosCompleted >= 3) {  // ✅ FIXED: Require 3 scenarios
...
const handleDoor2Complete = () => {
  console.log('🚪 Door 2 completed - starting final celebration!');
  ...
  sceneActions.updateState({
    ...
    showAlwaysText: true, // ✅ NEW: Show animated text
  ...
  // Hide animated text after animation
  setTimeout(() => {
    sceneActions.updateState({ 
...
{sceneState.phase === SCENE_PHASES.CHARACTER_SELECT && (
  ...
  {/* ✅ ADD THIS: Character Speech Bubble */}
...
{sceneState.scenarioPhase === 'uncertainty' && (
  <div className="helper-instruction">
    <p>🤝 {getCurrentScenario().helpRequest}</p>
  </div>
)}

{/* ✅ NEW: Helper Badge 
{sceneState.helperBadgeVisible && (
        ...
        >
          <DraggableItem
            ...
            data={{ 
              symbolId: symbol.id, 
              isWise: isWise, 
              type: isWise ? 'wise-symbol' : 'unwise-symbol'
            }}
            disabled={!sceneState.symbolsClickable}
          >
            <div className={`cave-symbol-item ${isWise ? 'wise-symbol' : 'unwise-symbol'}`}>
              <img src={symbol.image} alt={symbol.name} />
               {/* ✅ ADD THIS: Symbol name text */}
