...
// At the top of SarvakaryeshuSarvadaV2.jsx, add:
import HelperInteraction from '../../../../lib/components/interactive/HelperInteraction';
// Add to imports:
import { MissionStoryManager, MISSION_STORIES, AnimatedStoryPlayer } from './animated-problem-scenarios';
...
// ✅ COMPLETE Game Flow with Game 2
const SCENE_PHASES = {
  DOOR1_ACTIVE: 'door1_active',                    // → FIRST: Door 1
  DOOR1_COMPLETE: 'door1_complete',
  CHARACTER_SELECT: 'character_select',             // → After Door 1
  ...
  GAME2_MISSION_INTRO: 'game2_mission_intro',       // → Mission introduction
  GAME2_HELPER_SELECTION: 'game2_helper_selection', // → Choose helpers

GAME2_PROBLEM_ANIMATION: 'game2_problem_animation',     // NEW: Show struggling
  GAME2_HELPER_JOURNEY: 'game2_helper_journey',           // NEW: Helper travels  
  GAME2_PROBLEM_SOLVING: 'game2_problem_solving',         // NEW: Step-by-step help
  GAME2_GRATITUDE: 'game2_gratitude',                     // NEW: Natural thanks

  GAME2_HELPER_INTERACTION: 'game2_helper_interaction', // → Mini-interactions
  GAME2_HELPER_ACTIVATED: 'game2_helper_activated', // → Ready to drag
  GAME2_MISSION_COMPLETE: 'game2_mission_complete', // → Mission done
  DOOR2_ACTIVE: 'door2_active',                     // → After 3 helper missions
...
// In SarvakaryeshuSarvadaV2.jsx, update the HELPER_MISSIONS object:
    ...
    <ErrorBoundary>
      <SceneManager
        ...
        initialState={{
          ...
          // ✅ NEW: Game 2 Helper Mission System
          game2Missions: [], // 3 random missions from 6 available
          currentMission: 0,
          currentMissionData: null,
          missionHelpers: [], // 3 random helpers from 4 available
          selectedHelper: null,
          helperInteractionType: null,
          helperInteractionComplete: false,
          missionBackground: 'before', // before/after
          helperActivated: false,
          missionsCompleted: 0,
          helperDragStarted: false,
          helperDragSuccess: false,
...
const [missionStoryManager] = useState(() => new MissionStoryManager());
  ...
  // ✅ NEW: Generate random missions for Game 2
...
const generateRandomHelpers = (missionId) => {
  console.log('🎭 Generating helpers for mission:', missionId);
  ...
  if (!mission.helpers || mission.helpers.length === 0) {
    console.error('❌ No helpers defined for mission:', missionId);
    return [];
  }
  
  // Shuffle and take 3 random helpers
  const shuffled = [...mission.helpers].sort(() => 0.5 - Math.random());
  const selected = shuffled.slice(0, 3);
  
  console.log('✅ Selected helpers:', selected.map(h => h.name));
  return selected;
};
  ...
  const completeCurrentScenario = () => {
    ...
    if (scenariosCompleted >= 3) {
      // All 3 scenarios complete - go to Game 2
...
const completeGame1 = () => {
  // Shows "In All Tasks" text
  sceneActions.updateState({
    phase: SCENE_PHASES.SARVAKARYESHU_LEARNING,
    showInAllTasksText: true
  });
  
  // ✅ CHANGE: Go to Door 2 instead of Game 2
  setTimeout(() => {
    sceneActions.updateState({ 
      showInAllTasksText: false,
      phase: SCENE_PHASES.DOOR2_ACTIVE  // ← Go to Door 2
    });
  }, 4000);
};

// REPLACE current initializeGame2():
const initializeGame2 = () => {
  console.log('🎭 Initializing Game 2 - Helper Academy');
  
  const randomMissions = generateRandomMissions();
  const firstMissionId = randomMissions[0];
  const firstMissionData = HELPER_MISSIONS[firstMissionId];
  
  sceneActions.updateState({
    game2Missions: randomMissions,
    currentMission: 0,
    currentMissionData: firstMissionData,
    phase: SCENE_PHASES.GAME2_PROBLEM_ANIMATION  // ← Start with problem animation
  });
  
  // Start animated problem sequence
  startAnimatedProblemSequence(firstMissionId);
};

// NEW function:
const startAnimatedProblemSequence = async (missionId) => {
  console.log(`🎭 Starting animated problem sequence for ${missionId}`);
  
  // Play problem sequence (10 seconds)
  await missionStoryManager.startMissionStory(missionId, 'mission-container');
  
  // Move to helper selection
  setTimeout(() => {
    const missionHelpers = generateRandomHelpers(missionId);
    sceneActions.updateState({
      missionHelpers: missionHelpers,
      phase: SCENE_PHASES.GAME2_HELPER_SELECTION
    });
  }, 10000);
};

  // ✅ NEW: Game 2 Functions

const startGame2FirstMission = () => {
  console.log('🌟 Starting first helper mission - data should already be set');
  
  // ✅ Just verify data exists
  if (!sceneState.currentMissionData) {
    console.error('❌ No mission data found, reinitializing...');
    initializeGame2();
    return;
  }
  
  console.log('✅ Mission data exists:', sceneState.currentMissionData.name);
  console.log('✅ Helpers exist:', sceneState.missionHelpers?.length || 0);
};

  const handleHelperClick = (helper) => {
    console.log(`🎭 Helper selected: ${helper.name}`);
    
    setShowSparkle('helper-selected');
    
    sceneActions.updateState({
      selectedHelper: helper,
      helperInteractionType: helper.interaction.type,
      phase: SCENE_PHASES.GAME2_HELPER_INTERACTION
    });
    
    setTimeout(() => setShowSparkle(null), 1000);
  };

const handleHelperInteraction = (helperId) => {
  console.log(`✨ Helper interaction completed: ${helperId}`);
  
  sceneActions.updateState({
    helperInteractionComplete: true,
    helperActivated: true,
    phase: SCENE_PHASES.GAME2_PROBLEM_SOLVING,  // ✅ CHANGED
    missionBackground: 'after'  // ✅ ADDED
  });
  
  // Start immediate problem solving
  startProblemSolvingSequence();  // ✅ CHANGED
};

// NEW function:
const startHelperJourney = () => {
  console.log('🚀 Starting helper journey animation');
  
  if (showMessage) {
    showMessage(`${sceneState.selectedHelper?.name} is traveling to help!`, {
      duration: 3000,
      animation: 'gentle',
      position: 'top-center'
    });
  ...
  // Journey animation lasts 3 seconds
  setTimeout(() => {
    sceneActions.updateState({
      phase: SCENE_PHASES.GAME2_PROBLEM_SOLVING
    });
    startProblemSolvingSequence();
  }, 3000);
};

  const startProblemSolvingSequence = async () => {
  console.log('🔧 Starting step-by-step problem solving');
  
  const missionId = sceneState.currentMissionData?.id;
  const helperId = sceneState.selectedHelper?.id;
  
  // Play solving sequence (15 seconds)
  await missionStoryManager.playSolvingSequence(helperId);
  
  // Show gratitude
  setTimeout(() => {
    sceneActions.updateState({
      phase: SCENE_PHASES.GAME2_GRATITUDE,
      missionBackground: 'after'
    });
    startGratitudeSequence();
  }, 15000);
};

const startGratitudeSequence = async () => {
  console.log('💝 Starting gratitude sequence');
  
  // Play gratitude sequence (5 seconds)
  await missionStoryManager.playGratitudeSequence();
  ...
  setTimeout(() => {
    ...
    completeCurrentHelperMission();
  }, 5000);
};

  const handleHelperDragToMission = () => {
    console.log('🚀 Helper sent to help person');
    
    setShowSparkle('helper-success');
    
    // Transition background from before to after
    sceneActions.updateState({
      missionBackground: 'after',
      helperDragSuccess: true,
      missionTransitioning: true
    });
    
    // Show gratitude message
    setTimeout(() => {
      const gratitudeMessage = sceneState.currentMissionData?.gratitude || 'Thank you for your help!';
      
      if (showMessage) {
        showMessage(gratitudeMessage, {
          duration: 3000,
    ...
    // Complete mission or continue to next
    setTimeout(() => {
      completeCurrentHelperMission();
    }, 5000);
  };

  const completeCurrentHelperMission = () => {
    const nextMission = (sceneState.currentMission || 0) + 1;
    const missionsCompleted = (sceneState.missionsCompleted || 0) + 1;
    
    console.log(`✅ Helper mission ${missionsCompleted} completed`);
    ...
    if (missionsCompleted >= 3) {
      // All 3 missions complete - go to Door 2
      setTimeout(() => completeAllHelperMissions(), 2000);
    } else {
      // Start next mission
      setTimeout(() => startNextHelperMission(nextMission), 2000);
    }
  };

  const startNextHelperMission = (missionIndex) => {
    console.log(`🎯 Starting helper mission ${missionIndex + 1}`);
    
    const missionId = sceneState.game2Missions[missionIndex];
    ...
    sceneActions.updateState({
      ...
      helperActivated: false,
      helperDragSuccess: false,
      missionBackground: 'before',
      phase: SCENE_PHASES.GAME2_MISSION_INTRO
    });
...
const completeAllHelperMissions = () => {
  console.log('🌟 All helper missions completed - final celebration!');
  
  // ✅ CHANGE: Go directly to completion
  sceneActions.updateState({
    helperMissionsCompleted: 3,
    phase: SCENE_PHASES.COMPLETE,  // ← Direct to completion
    completed: true,
    stars: 10,
    progress: {
      percentage: 100,
...
const handleDoor2Complete = () => {
  ...
  // ✅ CHANGE: Start Game 2 instead of ending
  setTimeout(() => {
    sceneActions.updateState({ 
      showAlwaysText: false,
      phase: SCENE_PHASES.GAME2_TRANSFORMATION,  // ← NOW start Game 2
      goldenHelperStatus: true
    });
    
    setTimeout(() => initializeGame2(), 3000);
  }, 3000);
};
      ...
      >
        <div className="scene-container" data-phase={sceneState.phase}>
          ...
          >
            ...
            {/* 🔧 COMPACT DEBUG PANEL - Right Side */}
<div style={{
  position: 'absolute',
  top: '70px',
  right: '10px',
  ...
  <button 
    onClick={() => {
      ...
      setTimeout(() => initializeGame2(), 2000);
    }}
    style={{
      background: '#FFD700',
      color: '#000',
  ...
  <button 
    onClick={() => {
      console.log('🔧 DEBUG: Direct Helpers');
      const randomMissions = generateRandomMissions();
      const firstMissionId = randomMissions[0];
      const firstMissionData = HELPER_MISSIONS[firstMissionId];
      const firstMissionHelpers = generateRandomHelpers(firstMissionId);
      ...
      sceneActions.updateState({
        ...
        currentMission: 0,
        currentMissionData: firstMissionData,
        missionHelpers: firstMissionHelpers,
        missionBackground: 'before',
        phase: SCENE_PHASES.GAME2_HELPER_SELECTION
      });
    }}
  ...
  <button 
    onClick={() => {
      const grandparentsHelpers = generateRandomHelpers('grandparents');
      sceneActions.updateState({
        selectedCharacter: 'girl',
        game1Completed: true,
        goldenHelperStatus: true,
        game2Missions: ['grandparents', 'dog', 'construction'],
        currentMission: 0,
        currentMissionData: HELPER_MISSIONS.grandparents,
        missionHelpers: grandparentsHelpers,
        missionBackground: 'before',
        phase: SCENE_PHASES.GAME2_HELPER_SELECTION
      });
    }}
  ...
  >
    🔄
           ...
           {/* ✅ UPDATED: Character Selection - Clean version */}
            ...
            {/* ✅ NEW: Game 2 - Helper Academy Components */}
            
            {/* Golden Helper Transformation */}
            {sceneState.phase === SCENE_PHASES.GAME2_TRANSFORMATION && (
              <div className="golden-helper-transformation">
                <div className="transformation-glow"></div>
                <div className="transformation-text">
                  <h2>You are now Ganesha's Golden Helper!</h2>
                  <p>Time to help others with divine wisdom</p>
                </div>
              </div>
            )}

            {/* Mission Introduction */}
            {sceneState.phase === SCENE_PHASES.GAME2_MISSION_INTRO && sceneState.currentMissionData && (
              <div className="mission-intro-overlay">
                <div className="mission-intro-card">
                  <h3>{sceneState.currentMissionData.name}</h3>
                  <p>{sceneState.currentMissionData.intro}</p>
                  <div className="mission-counter">Mission {(sceneState.currentMission || 0) + 1} of 3</div>
                </div>
              </div>
            )}

          
{sceneState.phase === SCENE_PHASES.GAME2_PROBLEM_ANIMATION && (
  <div 
    id="mission-container" 
    className="mission-story-container"
    style={{
      position: 'absolute',
      top: '15%',
      left: '25%',
  ...
  >
    <img 
      src={sceneState.currentMissionData?.backgrounds.before} 
      alt="Mission problem"
...
{sceneState.phase === SCENE_PHASES.GAME2_HELPER_JOURNEY && sceneState.selectedHelper && (
  <div className="helper-journey-animation">
    <div 
      className="traveling-helper"
      style={{
        position: 'absolute',
        top: '70%',
        left: '20%',
        ...
        height: '60px',
        borderRadius: '50%',
        backgroundColor: sceneState.selectedHelper.color || '#FFD700',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        fontSize: '24px',
        animation: 'helperJourney 3s ease-in-out forwards',
        zIndex: 20
      }}
    >
      {sceneState.selectedHelper.icon}
    </div>
    ...
    <div 
      ...
      style={{
        ...
        animation: 'trailSparkle 3s ease-in-out',
        zIndex: 19
      }}
    />
  </div>
            ...
            {/* Helper Mission Background */}
            {(sceneState.phase === SCENE_PHASES.GAME2_MISSION_INTRO ||
              sceneState.phase === SCENE_PHASES.GAME2_HELPER_SELECTION ||
              sceneState.phase === SCENE_PHASES.GAME2_HELPER_INTERACTION ||
              sceneState.phase === SCENE_PHASES.GAME2_HELPER_ACTIVATED ||
              sceneState.phase === SCENE_PHASES.GAME2_MISSION_COMPLETE) && 
              sceneState.currentMissionData && (
              <div className="helper-mission-window">
                <img 
                  src={sceneState.currentMissionData.backgrounds[sceneState.missionBackground]} 
                  alt={`${sceneState.currentMissionData.name} - ${sceneState.missionBackground}`}
                  className={`mission-background ${sceneState.missionBackground} ${sceneState.missionTransitioning ? 'transitioning' : ''}`}
                />

                {sceneState.phase === SCENE_PHASES.GAME2_PROBLEM_SOLVING && (
  <div className="problem-solving-sequence">
    <div className="helper-working" style={{
      position: 'absolute',
      top: '20%',
      right: '20%',
      width: '80px',
      ...
      justifyContent: 'center',
      fontSize: '32px',
      animation: 'helperWorking 2s ease-in-out infinite',
      zIndex: 15
    }}>
      {sceneState.selectedHelper?.icon}
    </div>
    
    <div className="solving-effects" style={{
      position: 'absolute',
      top: '15%',
      left: '25%',
      width: '50%',
      ...
      animation: 'solvingGlow 3s ease-in-out infinite',
      zIndex: 10
    }} />
  </div>
)}

{sceneState.phase === SCENE_PHASES.GAME2_GRATITUDE && (
  <div className="gratitude-sequence">
    <div className="mission-gratitude-bubble">
      <p>{sceneState.currentMissionData?.gratitude}</p>
    </div>
    
    <div className="happy-transformation" style={{
      position: 'absolute',
      top: '0',
      left: '0',
      width: '100%',
      height: '100%',
      background: 'linear-gradient(45deg, rgba(255,215,0,0.1), rgba(255,140,66,0.1))',
      animation: 'happyGlow 3s ease-out',
      zIndex: 5
    }} />
  </div>
)}
                
                {/* Mission Complete Gratitude */}
                {sceneState.missionBackground === 'after' && sceneState.helperDragSuccess && (
                  <div className="mission-gratitude-bubble">
                    <p>{sceneState.currentMissionData.gratitude}</p>
                  </div>
                )}
...
{/* Helpers Around Scene - NEW LAYOUT */}
{sceneState.phase === SCENE_PHASES.GAME2_HELPER_SELECTION && 
 sceneState.missionHelpers && 
 sceneState.missionHelpers.length > 0 && 
 sceneState.missionHelpers.map((helper, index) => (
  <ClickableElement
    key={helper.id}
    id={`helper-${helper.id}`}
    ...
    zone="helper-zone"
  >
    <div className={`helper-around-scene position-${index + 1}`}>
...
{/*{sceneState.phase === SCENE_PHASES.GAME2_HELPER_INTERACTION && sceneState.selectedHelper && (
  <div className="helper-interaction-zone">
    <div className="interaction-instruction">
      <p>{sceneState.selectedHelper.interaction.instruction}</p>
      <ClickableElement
        id="complete-interaction"
        onClick={() => handleHelperInteraction(sceneState.helperInteractionType)}
        completed={false}
...
{sceneState.phase === SCENE_PHASES.GAME2_HELPER_INTERACTION && sceneState.selectedHelper && (
  <HelperInteraction
    helperId={sceneState.selectedHelper.id}
    onInteractionComplete={(helperId, phrase) => {
      console.log(`✅ ${helperId} interaction complete: ${phrase}`);
      sceneActions.updateState({
        helperInteractionComplete: true,
        helperActivated: true,
        phase: SCENE_PHASES.GAME2_HELPER_ACTIVATED
      });
      
      if (showMessage) {
        showMessage(`${sceneState.selectedHelper.name} is ready to help!`, {
          duration: 2000,
          animation: 'gentle',
          position: 'top-center'
        });
    ...
    onInteractionFailed={(helperId) => {
      console.log(`❌ ${helperId} interaction failed`);
    }}
  />
)}

            {/* Helper Activated - Ready to Drag */}
            {sceneState.phase === SCENE_PHASES.GAME2_HELPER_ACTIVATED && sceneState.selectedHelper && (
              <div className="helper-activated-zone">
                <div className="activated-helper">
                  <div 
                    className="helper-activated-placeholder"
                    style={{ backgroundColor: sceneState.selectedHelper.placeholder }}
                  >
                    <span className="helper-activated-icon">{sceneState.selectedHelper.icon}</span>
                    <div className="activation-glow"></div>
                  </div>
                  <p className="activation-message">{sceneState.selectedHelper.name} is ready to help!</p>
                </div>
                
                <div className="drag-instruction">
                  <p>Drag the helper to the person who needs help!</p>
                  <ClickableElement
                    id="send-helper"
                    onClick={handleHelperDragToMission}
                    completed={false}
                    zone="mission-zone"
                  >
                    <div className="send-helper-button">
            ...
            {/* ✅ FOURTH: Door 2 Component - Shows AFTER 3 helper missions complete */}
