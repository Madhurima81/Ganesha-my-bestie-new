...
const SCENE_PHASES = {
  // Door & Character phases (keep existing)
  DOOR1_ACTIVE: 'door1_active',
  DOOR1_COMPLETE: 'door1_complete', 
  CHARACTER_SELECT: 'character_select',
  
  // Game 1 phases (keep existing)
  ...
  // Game 2 phases (SIMPLIFIED)
  GAME2_TRANSFORMATION: 'game2_transformation',
  GAME2_MISSION_INTRO: 'game2_mission_intro',
  MISSION_UNCERTAINTY: 'mission_uncertainty',         // NEW: Problem speech
  MISSION_CLARITY: 'mission_clarity',                // NEW: Helpers appear  
  MISSION_CHOOSING: 'mission_choosing',              // NEW: Helper popup
  MISSION_SUCCESS: 'mission_success',                // NEW: Animation
  GANESHA_GOLDEN_BLESSING: 'ganesha_golden_blessing', // NEW: Divine moment
  GAME2_MISSION_COMPLETE: 'game2_mission_complete',
  
  // Final phases (keep existing)
    ...
    <ErrorBoundary>
      <SceneManager
        ...
        initialState={{
          ...
          // ✅ KEEP these simple states:
selectedHelper: null,              // Like selectedSymbol
currentMission: 0,
currentMissionData: null,
missionHelpers: [],
missionBackground: 'before',       // before/after
helperPopupVisible: false,         // NEW: for center popup
helperMessage: null,               // NEW: helper speech

// ADD these Game 1 style states:            // NEW: helper speech bubble text
helpersVisible: 'none',            // NEW: 'none'/'faint'/'clear' (like symbolsVisible)
helpersClickable: false,           // NEW: like symbolsClickable
showHelperProblem: false,          // NEW: like showProblemDeclaration
helperProblemShown: false,         // NEW: like problemDeclarationShown
ganeshaPresent: false,             // NEW: for divine blessing moment
divineMessage: null,               // NEW: Ganesha's blessing text
...
// ✅ NEW: Clean and simple (Game 1 style)
...
// ✅ REPLACE complex handleHelperClick with Game 1 style
const handleHelperClick = (helper) => {
  console.log(`🤖 Helper clicked: ${helper.name}`);
  
  sceneActions.updateState({
    selectedHelper: helper,
    helperPopupVisible: true,
    helperMessage: helper.phrase || `I'm ${helper.name}, ready to help!`,
...
// ✅ NEW: Simple helper activation (like Game 1's scene click)
const handleHelperConfirm = () => {
  if (!sceneState.selectedHelper) return;
  
  console.log(`✨ Helper activated: ${sceneState.selectedHelper.name}`);
  ...
  // After animation, show Ganesha blessing
  setTimeout(() => {
    setShowSparkle(null);
    showGaneshaGoldenBlessing();
  }, 3500); // Animation duration
};


// ✅ REPLACE complex sequences with simple blessing
const showGaneshaGoldenBlessing = () => {
  console.log('🕉️ Divine partnership moment');
  ...
  // Complete mission after blessing
  setTimeout(() => {
    sceneActions.updateState({
    ...
    setTimeout(() => completeCurrentMission(), 2000);
  }, 3000);
};

// ✅ ADD this missing function:
const handleAllMissionsComplete = () => {
  console.log('🌟 All helper missions completed!');
...
// ✅ SIMPLIFIED: Clean mission progression
const completeCurrentMission = () => {
  ...
  console.log(`✅ Mission ${missionsCompleted} completed`);
  ...
  if (missionsCompleted >= 3) {
    // All missions complete → Door 2 or completion
    setTimeout(() => handleAllMissionsComplete(), 2000);
  } else {
    // Start next mission
...
const handleDoor2Complete = () => {
  console.log('🚪 Door 2 completed - Scene complete!');
  ...
  // Show "Always" text, then start Game 2
  setTimeout(() => {
    sceneActions.updateState({ 
    ...
    // Initialize Game 2 missions and start first one
    setTimeout(() => {
      const randomMissions = generateRandomMissions();
      sceneActions.updateState({
        game2Missions: randomMissions
      });
      startMission(0); // ✅ Use new function instead of initializeGame2()
    }, 3000);
  }, 3000);
};
...
{/*ADD: Helpers positioned around scene (copy symbol pattern)*/}
{(sceneState.phase === SCENE_PHASES.MISSION_CLARITY ||
  sceneState.phase === SCENE_PHASES.MISSION_CHOOSING) &&
  sceneState.helpersVisible !== 'none' && 
  sceneState.missionHelpers && 
  sceneState.missionHelpers.length > 0 && 
  sceneState.missionHelpers.map((helper, index) => (
    <div 
      key={helper.id}
      className={`cave-symbol-individual position-${index + 1} ${sceneState.helpersVisible} ${sceneState.helpersClickable ? 'clickable' : 'unclickable'}`}
    >
      <ClickableElement
        id={`helper-${helper.id}`}
        ...
        completed={false}
        zone="helper-zone"
        disabled={!sceneState.helpersClickable}
      >
...
{/*ADD: Simple mission intro (like character selection)*/}
{sceneState.phase === SCENE_PHASES.GAME2_MISSION_INTRO && sceneState.currentMissionData && (
  <div className="mission-intro-overlay">
    <div className="mission-intro-card">
      <h3>{sceneState.currentMissionData.name}</h3>
      ...
      <div className="mission-counter">
        Mission {(sceneState.currentMission || 0) + 1} of 3
      </div>
    </div>
  </div>
)}

{/*ADD: Problem speech (copy Game 1's pattern)*/}
{(sceneState.phase === SCENE_PHASES.MISSION_UNCERTAINTY ||
  sceneState.phase === SCENE_PHASES.MISSION_CLARITY ||
  sceneState.phase === SCENE_PHASES.MISSION_CHOOSING ||
  sceneState.phase === SCENE_PHASES.MISSION_SUCCESS) && (
  <>
    {/* Mission Window - Always Show */}
    <div className="scenario-window">
      <img 
        src={sceneState.currentMissionData?.backgrounds[sceneState.missionBackground]} 
        alt="Mission situation"
        className={`scenario-image ${sceneState.sceneClarity || 'uncertain'}`}
      />

      {/* Problem Declaration (like Game 1) */}
      {sceneState.showHelperProblem && (
        <div className="character-problem-speech">
          <p>{sceneState.currentMissionData?.problemSpeech}</p>
        </div>
      )}

      {/* Mission Gratitude */}
      {sceneState.missionBackground === 'after' && (
        <div className="mission-gratitude-bubble">
          <p>{sceneState.currentMissionData?.gratitude}</p>
        </div>
      )}
...
{/*ADD: Helper popup when clicked (copy character selection pattern)*/}
{sceneState.helperPopupVisible && sceneState.selectedHelper && (
  <div className="helper-popup-center">
    <div className="helper-popup-card">
      <div className="helper-image-large">
        <img 
          src={sceneState.selectedHelper.image} 
          alt={sceneState.selectedHelper.name}
        />
      </div>
      
      <div className="helper-info">
        <h3>{sceneState.selectedHelper.name}</h3>
        
        {/* Helper Speech Bubble */}
        <div className="helper-speech-bubble">
          <p>{sceneState.helperMessage}</p>
        </div>
        ...
        <ClickableElement
          ...
          completed={false}
          zone="activation-zone"
        >
          <div className="helper-activation-button">
            <span>Tap me to activate my power!</span>
          </div>
        </ClickableElement>
      </div>
    </div>
    
    {/* Close overlay by clicking other helpers */}
    <div className="popup-overlay" onClick={() => {
      // Allow clicking other helpers to change popup
    }} />
  </div>
)}
...
{/*ADD: Simple signature animation (when mission success)*/}
{sceneState.phase === SCENE_PHASES.MISSION_SUCCESS && sceneState.selectedHelper && (
  <HelperSignatureAnimation
    helperId={sceneState.selectedHelper.id}
    fromPosition={{ x: 20, y: 20 }}  // Helper position
    toPosition={{ x: 50, y: 50 }}    // Mission window center
    onAnimationComplete={() => {
      console.log('✨ Helper animation complete');
      showGaneshaGoldenBlessing();
    }}
    onAnimationStart={() => {
      console.log('🎭 Helper animation started');
    }}
  />
)}

{/*UPDATE: Simplify existing helper mission window*/}
{(sceneState.phase === SCENE_PHASES.GAME2_MISSION_INTRO ||
  sceneState.phase === SCENE_PHASES.MISSION_UNCERTAINTY ||
  sceneState.phase === SCENE_PHASES.MISSION_CLARITY ||
  sceneState.phase === SCENE_PHASES.MISSION_CHOOSING ||
  ...
  <div className="helper-mission-window">
    <img 
      src={sceneState.currentMissionData.backgrounds[sceneState.missionBackground]} 
      alt={`${sceneState.currentMissionData.name} - ${sceneState.missionBackground}`}
      className={`mission-background ${sceneState.missionBackground}`}
    />
  </div>
)}
