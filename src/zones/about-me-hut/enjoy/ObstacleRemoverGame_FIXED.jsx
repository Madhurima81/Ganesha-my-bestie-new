...
const DreamsWishesGameContent = ({ sceneState, sceneActions, isReload, onComplete, onNavigate, onBack }) => {
  ...
  // ── MAIN PHASE-ENTRY VO EFFECT ──────────────────────────────────────────────
  // Triggers VO when phase changes (both reload and normal transitions)
  useEffect(() => {
    const celebrationPhases = ['wish1-complete', 'wish2-complete', 'wish3-complete', 'all-wishes-complete'];

    // Reload stabilization guard: block stale VO from pre-restore phases
    ...
    // Celebration phase race guard: prevent stale celebration VO after reload
    ...
    // First hydration guard: skip celebration VO on initial mount
    ...
    if (sceneState.gamePhase === 'comparison-card' && !phaseVoiceRef.current.comparison) {
      ...
      speakLine(VOICE_LINES.comparison, { moment: 'thinking' });
    }
  ...
  // Reload restore can occasionally drop the first TTS call in this scene.
  // Retry once if speech did not start.
  const replayPhaseVoiceOnReload = useCallback((voiceKey, line, moment = 'encouragement') => {
    ...
    safeSetTimeout(() => {
      attemptSpeak();
      safeSetTimeout(() => {
        if (!started && !isSpeakingRef.current && isAudioOn) {
          attemptSpeak();
        }
      }, 700);
    }, 500);
  }, [isAudioOn, safeSetTimeout]);
  ...
  // ── PHASE-CHANGE VO ─────────────────────────────────────────────────────
  useEffect(() => {
    if (reloadHandledRef.current || !sceneState?.gamePhase) return;

    const phase = sceneState.gamePhase;
    const phaseKey = `phase-${phase}`;

    if (phaseVoiceRef.current[phaseKey]) return;

    const voLine = getResumeVoiceLine(phase);

    if (voLine && isAudioOn) {
      phaseVoiceRef.current[phaseKey] = true;

      const moment = ['wish1-intro', 'wish2-intro', 'wish3-intro', 'all-wishes-complete', 'dream-drawing'].includes(phase)
        ? 'story'
        : 'encouragement';

      speakLine(voLine, { moment });
    }
  }, [sceneState?.gamePhase, isAudioOn, getResumeVoiceLine, speakLine]);
  ...
  useEffect(() => {
    if (isReload && !reloadHandledRef.current) {
      ...
      if (gamePhase === 'dream-clouded' || dreamRevealed) {
        ...
        phaseVoiceRef.current = {};
        if (dreamRevealed) replayPhaseVoiceOnReload('dreamRevealed', VOICE_LINES.dreamRevealed, 'celebration');
        else replayPhaseVoiceOnReload('dreamClouded', VOICE_LINES.dreamClouded, 'story');
        return;
      }
