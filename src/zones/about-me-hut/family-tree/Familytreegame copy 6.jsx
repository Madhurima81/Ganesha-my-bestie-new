...
const FamilyTreeGame = ({ onComplete, onBack, onNavigate }) => {
  // Get profile and session info for localStorage tracking
  const activeProfileId = localStorage.getItem('activeProfileId');
  const zoneId = 'about-me-hut';
  const sceneId = 'family-tree';
...
const [resumeMessage, setResumeMessage] = useState('');
  ...
  // Save progress to localStorage for Continue/Replay detection
// Save progress to localStorage for Continue/Replay detection
useEffect(() => {
  if (!activeProfileId) return;
  
  const tempKey = `temp_session_${activeProfileId}_${zoneId}_${sceneId}`;
  
  const sessionData = {
    phase: gamePhase,
    placedMembers: placedGaneshaMembers.size,
    childFamilyCount: childFamily.length,
    completed: showSceneCompletion,
    stars: 0
  };
  
  localStorage.setItem(tempKey, JSON.stringify(sessionData));
}, [gamePhase, placedGaneshaMembers, childFamily, showSceneCompletion, activeProfileId]);


// Helper function to calculate progress percentage
const calculateProgress = () => {
  if (showSceneCompletion) return 100;
  if (gamePhase === 'intro') return 0;
  if (gamePhase === 'ganeshaBuild') {
    return (placedGaneshaMembers.size / 4) * 50; // 0-50%
  }
  if (gamePhase === 'childFamily') {
    return 50 + (childFamily.length / 3) * 50; // 50-100%
  }
  return 0;
};
