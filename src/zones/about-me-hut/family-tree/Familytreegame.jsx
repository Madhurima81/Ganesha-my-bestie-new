...
const FamilyTreeGameContent = ({
  ...
  }, [sceneState.showNameModal, sceneState.currentFamilyType, namePromptPlayed]);

  // ========================================
  // CHILD PHASE: Play progress full VO when at least 1 member added
  // (Play once when Done button becomes visible)
  // ========================================
  const [childProgressFullPlayed, setChildProgressFullPlayed] = useState(false);

  useEffect(() => {
    if (
      sceneState.gamePhase === 'childInput' &&
      sceneState.childFamily.length > 0 &&
      !childProgressFullPlayed
    ) {
      setChildProgressFullPlayed(true);

      setTimeout(() => {
        playVoice('childProgressFull');
      }, 3000);
    }
  }, [sceneState.gamePhase, sceneState.childFamily.length, childProgressFullPlayed]);
  ...
  const handleChoiceSelection = (choice) => {
    ...
    if (choice.isCorrect) {
      ...
      // Play correct choice VO after deity name
      setTimeout(() => {
  ...
  const handleAddFamilyMember = () => {
    ...
    // Play member added VO for 2nd and 3rd members
    if (memberCount === 2 || memberCount === 3) {
      setTimeout(() => {
        playVoice('memberAdded');
      }, 500);
    }

    // Play progress VOs at gaps (after member added finishes)
    if (memberCount === 2) {
      setTimeout(() => {
        playVoice('childProgress1');
        setChildProgressCount(1);
      }, 2000);
    } else if (memberCount === 4) {
      setTimeout(() => {
        playVoice('childProgress3');
        setChildProgressCount(3);
      }, 2000);
    } else if (memberCount === 6) {
      setTimeout(() => {
        playVoice('childProgress5');
        setChildProgressCount(5);
      }, 2000);
    }
  };
