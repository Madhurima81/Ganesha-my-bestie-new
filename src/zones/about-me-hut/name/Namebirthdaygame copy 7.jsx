...
const handleFestivalClick = (festivalId) => {
    // If the info panel is already open for this card, ignore click
    if (sceneState.flippedCards.includes(festivalId)) return;

    const festival = festivals.find(f => f.id === festivalId);

    if (festival.correct) {
      // Correct! Move to success
      sceneActions.updateState({
          selectedFestival: festivalId,
          gamePhase: 'birthday-correct'
      });
    } else {
      // Wrong! Educational Moment -> Open Info Panel
      sceneActions.updateState({ 
        flippedCards: [festivalId] 
      });
      // Optional: Update hint text
      setInstructionMessage("Read the card to find out what this is! 👇");
    }
  };

  const handleCloseInfo = (e, festivalId) => {
    e.stopPropagation();
    // Close panel AND mark as wrong (gray out) now that they've read it
    sceneActions.updateState({ 
      flippedCards: [],
      wrongFestivals: [...sceneState.wrongFestivals, festivalId]
    });
      ...
      {sceneState.gamePhase === 'birthday-choice' && (
        <div className="birthday-choice-screen">
          ...
          <div className="birthday-speech-bubble">
            ...
            <span className="bubble-instruction-small">{instructionMessage || "Tap the cards to choose ✨"}</span>
          </div>
              ...
              return (
                <div key={festival.id} className={`birthday-card-container ${isFlipped ? 'container-active' : ''}`}>
                  ...
                  <button 
                    ...
                    onClick={(e) => { e.stopPropagation(); sceneActions.updateState({ flippedCards: [festival.id] }); }}
                    disabled={isWrong}
                    style={{ [isLeftColumn ? 'left' : 'right']: '10px' }}
                  >ⓘ</button>

                  {/* Info Panel (The Reveal) */}
                  {isFlipped && (
                    <div className={`birthday-info-panel ${isLeftColumn ? 'slide-left' : 'slide-right'}`}>
                      <button className="birthday-info-close" onClick={(e) => handleCloseInfo(e, festival.id)}>✕</button>
                      <div className="birthday-info-content">
                        {/* The REVEAL: Real Name */}
