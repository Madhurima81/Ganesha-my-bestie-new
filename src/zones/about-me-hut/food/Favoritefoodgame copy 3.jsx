        ...
        <div className="choice-screen">
          ...
          <div className="choices-container">
            {colors.map((color, index) => (
              <button
                key={color.id}
                className={`choice-card ${showShake === color.id ? 'shake' : ''} ${wrongChoices.has(color.id) ? 'wrong' : ''} bounce-gentle`}
                onClick={() => handleColorClick(color.id)}
                style={{ animationDelay: `${index * 0.2}s` }}
                disabled={wrongChoices.has(color.id)}
              >
                <div className="choice-image-container">
        ...
        <div className="choice-screen">
          ...
          <div className="choices-container">
            {activities.map((activity, index) => (
              <button
                key={activity.id}
                className={`choice-card ${showShake === activity.id ? 'shake' : ''} ${wrongChoices.has(activity.id) ? 'wrong' : ''} bounce-gentle`}
                onClick={() => handleActivityClick(activity.id)}
                style={{ animationDelay: `${index * 0.2}s` }}
                disabled={wrongChoices.has(activity.id)}
              >
                <div className="choice-image-container">
