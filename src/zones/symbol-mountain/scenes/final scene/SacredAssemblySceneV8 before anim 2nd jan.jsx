...
/*const handleCorrectPlacement = (symbol) => {
  ...
  console.log('✅ Symbol placed:', symbol.name, '| Total placed:', placedCount); // DEBUG
  ...
  // Check if game is complete BEFORE incrementing round
  if (placedCount === 8) {
    // All 8 symbols placed! Trigger final celebration
    console.log('🎉 All 8 symbols placed! Starting celebration!'); // DEBUG
    safeSetTimeout(() => {
      triggerFinalCelebration();
    }, 2000);
} else {
  // More rounds to go - move to next round
  const nextRound = (sceneState.currentRound || 0) + 1;
  console.log('➡️ Moving to next round:', nextRound); // DEBUG
  
  safeSetTimeout(() => {
    // Update round number in state
    sceneActions.updateState({
      currentRound: nextRound
    });
    
    // Start next round immediately with the new round number
    safeSetTimeout(() => {
      startNextRound(nextRound); // Pass the round number directly
    }, 100);
  }, 2000);
}
};*/
