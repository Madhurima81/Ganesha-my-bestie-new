...
const SymbolMountainSceneContent = ({ 
  ...
  const handleNoteClick = (noteId) => {
    ...
    const newNoteStates = { ...sceneState.musicalNoteStates, [noteId]: 'used' };
            ...
            {sceneState.musicalNotesVisible && (
              <>
                {musicalNoteData.map((noteData) => {
                  ...
                  const positions = { 'note1': { top: '17%', left: '27%' }, 'note2': { top: '22%', left: '47%' }, 'note3': { top: '19%', left: '67%' } };
                  const position = positions[noteData.id];
                  
                  return (
                    <div 
                      key={noteData.id}
                      className={`musical-note ${showHintGlow && isGolden && sceneState.showTuskAssemblyGame ? 'hint-glow' : ''}`}
                      onClick={() => { if (isGolden && sceneState.showTuskAssemblyGame) handleNoteClick(noteData.id); }}
                      style={{
                        position: 'absolute', top: position.top, left: position.left, width: '60px', height: '60px', zIndex: 45,
                        display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: isGolden ? 1 : 0.6,
                        cursor: isGolden && sceneState.showTuskAssemblyGame ? 'pointer' : 'default', transition: 'all 0.3s ease',
                        textShadow: isGolden ? '0 0 15px rgba(255, 215, 0, 0.8)' : 'none', fontSize: '40px',
