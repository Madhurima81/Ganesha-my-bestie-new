...
// Add mission images (create placeholders first)
import eyesBefore from '../tusk/assets/images/eye-before.png';
import eyesAfter from '../tusk/assets/images/eye-after.png';
import earsBefore from '../tusk/assets/images/ear-before.png';
import earsAfter from '../tusk/assets/images/ear-after.png';
...
const missionImages = {
  eyes: { before: eyesBefore, after: eyesAfter },
  ears: { before: earsBefore, after: earsAfter },
...
const [showHintGlow, setShowHintGlow] = useState(false);
  ...
  // Handle note click - flies to tusk automatically
const handleNoteClick = (noteId) => {
  console.log(`🎵 Note ${noteId} clicked - flying to tusk!`);
  ...
  if (noteState !== 'golden') {
    console.log('🚫 Note is not golden yet');
    return;
  }
  ...
  // Visual feedback - note flies to tusk
  setShowSparkle('tusk-feeding');
  setTimeout(() => setShowSparkle(null), 1500);
  
  // Check if complete
  if (newTuskPower >= 3) {
    console.log('🎉 Tusk fully powered - starting assembly');
...
const shouldEnableHints = () => {
  const disabledPhases = [
    PHASES.ALL_COMPLETE,
    PHASES.EYES_COMPLETE,
    PHASES.EARS_COMPLETE
  ];
  
  // Disable during games, modals, or missions
...
const getHintConfigs = () => [
  ...
  {
    id: 'tusk-hint',
    message: 'Click notes to tusk!', // ✅ Shortened
...
}, [sceneState?.phase, sceneState?.welcomeShown, showPowerModal, showPowerMission, sceneState?.showEyesTelescopeGame, sceneState?.showEarsRhythmGame]);
...
const handleContinueLearning = () => {
  ...
  if (symbolKey === 'eyes') {
    ...
    // Complete scene - straight to fireworks (NO GameCoach)
    setShowSparkle('final-fireworks');
    sceneActions.updateState({
      phase: PHASES.ALL_COMPLETE,
      stars: 9,
  ...
  <>
    ...
    <div style={{
      ...
      <div style={{
        ...
        Discover <strong>Eyes, Ears & Tusk</strong> to unlock their magic and rescue trapped animals
      </div>
...
{!showPowerModal && !showPowerMission && sceneState?.welcomeShown && (
  <>
    {sceneState.phase === PHASES.EYES_GAME && 
     ...
     !sceneState.eyesGameComplete && (
      <div className="phase-header">
        CLICK THE DIVINE EYES! 
      </div>
    )}
    
    {/*{sceneState.showEyesTelescopeGame && (
      <div className="phase-header">
        USE THE TELESCOPE! Find all 4 instruments!
      </div>
    )}*/}
    ...
    {sceneState.earsVisible && 
     ...
     !sceneState.earsGameComplete && (
      <div className="phase-header">
        CLICK THE SACRED EARS!
      </div>
    )}
    
    {/*{sceneState.showEarsRhythmGame && (
      <div className="phase-header">
        MATCH THE RHYTHM! Play the sequence!
      </div>
    )}*/}
    
    {sceneState.showTuskAssemblyGame && 
     !sceneState.ganeshaComplete &&
     Object.values(sceneState.musicalNoteStates || {}).some(state => state === 'golden') && (
      <div className="phase-header">
        FEED THE TUSK! Click the notes!
      </div>
    )}
  </>
)}
  ...
  className={`eyes-symbol-container ${sceneState.eyesGameComplete ? 'completed' : 'active'} 
    ${showHintGlow && sceneState.phase === PHASES.EYES_GAME && !sceneState.showEyesTelescopeGame ? 'hint-glow' : ''}`}
...
>
  ...
  className={`ears-symbol-container ${sceneState.earsGameComplete ? 'completed' : 'active'} materialized
    ${showHintGlow && sceneState.earsVisible && !sceneState.showEarsRhythmGame ? 'hint-glow' : ''}`}
...
{musicalNoteData.map((noteData, index) => {
  ...
  return (
    <div 
      ...
      className={showHintGlow && isGolden && sceneState.showTuskAssemblyGame ? 'hint-glow' : ''}
      style={{
        position: 'absolute',
        top: position.top,
        left: position.left,
