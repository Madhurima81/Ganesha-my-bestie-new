...
const NewModakSceneContent = ({
  ...
  const hasProgress = () => {
    if (sceneState.clickedMounds && sceneState.clickedMounds.length > 0) return true;
    if (sceneState.collectedModaks && sceneState.collectedModaks.length > 0) return true;
    if (sceneState.rockFeedCount && sceneState.rockFeedCount > 0) return true;
    if (sceneState.discoveredSymbols && Object.keys(sceneState.discoveredSymbols).length > 0) return true;
    if (sceneState.phase !== PHASES.MOOSHIKA_SEARCH) return true;
    return false;
  };
  ...
  useEffect(() => {
    if (isReload && !reloadHandledRef.current) {
      ...
      if (sceneState.phase === PHASES.ALL_COLLECTED) {
        sceneActions.updateState({
          discoveredSymbols: { ...sceneState.discoveredSymbols, mooshika: true, modak: true }
        });
  ...
  // 1. WRAP THE ENTIRE RETURN IN GAMELAYOUT
  return (
    <GameLayout
      zoneId="symbol-mountain"
            ...
            <div className="modak-game-container">
              ...
              {showDiscoveryFlip1 && (
                <SimpleDiscoveryOverlay
                  ...
                  powerIcon={symbolMooshikaColored}
