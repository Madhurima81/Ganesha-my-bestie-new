...
const NewModakSceneContent = ({ 
  ...
  // CLICK HANDLERS... (Handlers omitted for brevity, logic unchanged)
  
  // ... (Paste all the handler functions like handleMoundClick, handleModakClick etc. here - they are unchanged) ...
const handleMoundClick = (moundIndex) => {
    ...
    if (moundIndex === sceneState.correctMound) {
      // 1. Show Sparkles immediately
      setShowSparkle('mooshika-found');
      
      // 2. Update State (Reveal Mooshika on the map behind the overlay)
      ...
      // 3. TRIGGER THE NEW OVERLAY (0.5s delay for smoothness)
      setTimeout(() => {
        setShowDiscoveryOverlay(true);
      }, 500);

      // 4. AUTO-ADVANCE SEQUENCE
      // Wait 4 seconds for kid to celebrate, then fade out
      setTimeout(() => {
        setIsDiscoveryFading(true); // Trigger CSS Fade Out
        
        setTimeout(() => {
          setShowDiscoveryOverlay(false);
          setIsDiscoveryFading(false);
          
          // 5. Trigger the next step (Glowing Symbol / Reward)
...
/>
            ...
            {showDiscoveryOverlay && (
              <div className={`discovery-overlay ${isDiscoveryFading ? 'fading-out' : ''}`}>
                <h1 className="discovery-title">You Found Mooshika!</h1>
                <p className="discovery-subtitle">He has something magical to share...</p>
                <img 
                  src={mooshika} 
                  alt="Mooshika Found" 
                  className="discovery-hero-img" 
