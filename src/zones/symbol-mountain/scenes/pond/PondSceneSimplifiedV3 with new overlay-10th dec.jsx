...
const completeSymbolLearning = (symbolKey, symbolData) => {
  ...
  const handleContinueLearning = () => {
    ...
    setTimeout(() => {
      sceneActions.updateState({
        goldenLotusVisible: true, // ✅ Golden BUD appears
        //elephantVisible: true,     // ✅ Elephant materializes
      phase: PHASES.GOLDEN_VISIBLE, // ✅ Wait for user to click golden lotus
      });
      
      // Optional: Sparkle effect for magic materialization
      setShowSparkle('golden-lotus-materialize');
...
if (bloomedCount === 3) {
  // First: show 3/3 in header (NO golden lotus yet)
  sceneActions.updateState({
    lotusStates,
    phase: PHASES.SOME_BLOOMED,
    // ❌ REMOVE goldenLotusVisible from here
    progress: {
      ...sceneState.progress,
      percentage: 30,
      starsEarned: 1
  ...
  // After 1 second: show golden lotus and trigger discovery
  setTimeout(() => {
    sceneActions.updateState({ 
      phase: PHASES.ALL_BLOOMED,
      currentFocus: 'waiting-for-golden',
      //goldenLotusVisible: true // ✅ Show golden lotus here
    });
  ...
  const handleElephantClick = () => {
    ...
    if (elephant) {
      ...
      safeSetTimeout(() => {
        ...
        safeSetTimeout(() => {
  // First: Bloom the golden lotus
  sceneActions.updateState({ 
    trunkActive: false,
    goldenLotusBloom: true, // ✅ ADD THIS - blooms golden lotus
    phase: PHASES.GOLDEN_BLOOM // ✅ ADD THIS - sets correct phase
  });
  setShowSparkle(null);
  
  // Then after 2 seconds: Trigger trunk discovery
  setTimeout(() => {
    triggerDiscoverySequence('trunk', 2000);
  }, 2000); // ✅ Give time to see the bloom animation
}, 1000);
