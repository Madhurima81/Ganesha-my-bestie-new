...
const handleFinishGame = () => {
  ...
  // Clear IN-PROGRESS state but keep completion data
  if (onSaveGameState) {
    onSaveGameState({
      gameId: gameConfig.id,
      gameMode: 'manual',
      ...
      playerInput: [],
      currentSequence: [],
      completedRounds: completedRounds,  // ⭐ KEEP THIS
      learnedSyllables: learnedSyllables,  // ⭐ KEEP THIS
      visualRewards: visualRewards,  // ⭐ KEEP THIS
      // Don't set cleared: true - just reset to initial state
    });
  }
};
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 20 }}>
      ...
      {!hideElements && gameState !== 'roundSelection' && gameState !== 'countdown' && gameState !== 'initializing' && (
        <div style={{ position: 'absolute', top: '20px', left: '50%', transform: 'translateX(-50%)', background: gameState === 'playing' ? 'rgba(33, 150, 243, 0.95)' : gameState === 'success' ? 'rgba(76, 175, 80, 0.95)' : centralElementGlowing ? 'rgba(255, 215, 0, 0.95)' : 'rgba(255, 152, 0, 0.95)', color: 'white', padding: '12px 24px', borderRadius: '25px', fontSize: '15px', fontWeight: 'bold', zIndex: 50 }}>
          {gameState === 'playing' && `Round ${selectedRound}: Listen carefully...`}
          {gameState === 'listening' && !centralElementGlowing && `Click the elephants! (${playerInput.length}/${currentSequence.length})`}
          {gameState === 'listening' && centralElementGlowing && '🌸 Click the lotus to finish!'}
