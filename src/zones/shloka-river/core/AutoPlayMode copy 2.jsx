  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 20 }}>
      ...
      {!hideElements && gamePhase !== 'phase_complete' && (
        <div style={{ position: 'absolute', top: '20px', left: '20%', transform: 'translateX(-50%)', background: centralElementGlowing ? 'rgba(255, 215, 0, 0.95)' : `${gameConfig.theme.primaryColor}E6`, color: 'white', padding: '10px 20px', borderRadius: '20px', fontSize: '14px', fontWeight: 'bold', zIndex: 50 }}>
          {isCountingDown ? `Get Ready... ${countdown}` : gamePhase === 'playing' ? 'Listen carefully...' : gamePhase === 'celebration' ? 'Beautiful! Well done!' : centralElementGlowing ? '🌸 Click the lotus to finish!' : `Click the elephants! (${playerInput.length}/${currentSequence.length})`}
        </div>
      )}
