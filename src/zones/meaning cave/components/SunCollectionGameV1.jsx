...
const handleObjectClick = (object, isStatic = false) => {
  ...
  if (activeTouches.current > 1) {
   ...
   // ✅ ADD THIS - Don't block fallback mode clicks
  if (!gameActive && !fallbackMode) {
    console.log('🚫 BLOCKED: Game not active');
    return;
  }
  if (gameWon) {
    console.log('🚫 BLOCKED: Game already won');
    return;
  }
  if (!object.isTarget) {
    console.log('🚫 BLOCKED: Not a target object');
    return;
  }
      ...
      {/* Cave play area */}
        ...
        onTouchStart={(e) => {
          ...
          if (e.touches.length > 1) {
            console.log('🚫 MULTI-TOUCH BLOCKED in Sun Game');
            e.preventDefault();
          }
        }}
        onTouchEnd={() => {
        ...
        style={{
          ...
          pointerEvents: gameWon ? 'none' : 'none', // ← Block during victory
          zIndex: 8
        }}
      >
        ...
        style={{
              ...
              pointerEvents: gameWon ? 'none' : 'auto', // ← Block during victory
