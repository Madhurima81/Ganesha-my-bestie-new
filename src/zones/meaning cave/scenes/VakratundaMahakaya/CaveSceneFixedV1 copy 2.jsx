...
const handleStoneClick = (stoneId) => {
  console.log(`🗿 Stone ${stoneId} clicked - NO PROTECTION`);
  
  // Find the stone
  const stone = sceneState.floatingStones.find(s => s.id === stoneId);
  if (!stone || stone.clicked) {
    console.log('Stone already clicked');
    return;
  }
  
  console.log(`✅ Processing stone ${stoneId}`);
  ...
  // Complete game when all 4 stones clicked
  if (newStonesClicked >= 4) {
    setTimeout(() => {
      completeGrowing();
    }, 1500);
...
<button 
  onClick={() => {
    ...
    if (currentMissionSymbol === 'vakratunda') {
      // Reset Tracing Game (ClickDotsPathGame)
      sceneActions.updateState({ 
        phase: CAVE_PHASES.TRACE_ACTIVE,  // ⭐ Go directly to tracing
        tracingStarted: true,  // ⭐ Mark as started
        tracingCompleted: false,  // ⭐ Not completed
        tracedPoints: [],  // ⭐ Clear traced points
        traceProgress: 0,  // ⭐ Reset progress to 0%
        traceQuality: 'good',
        currentPathSegment: 0,  // ⭐ Start from first segment
        segmentsCompleted: [],  // ⭐ Clear completed segments
        ganeshaVisible: false,  // ⭐ Hide mini ganesha
        ganeshaAnimation: 'breathing',
        ganeshaSize: 0.8,
        ganeshaGlow: 0.2,
        currentFocus: 'tracing'
      });
      
    } else if (currentMissionSymbol === 'mahakaya') {
      // Reset Growing Game (Floating Stones)
      sceneActions.updateState({ 
        phase: CAVE_PHASES.GROW_ACTIVE,  // ⭐ Go to growing phase
        growingStarted: true,  // ⭐ Mark as started
        growingCompleted: false,  // ⭐ Not completed
        stonesClicked: 0,  // ⭐ Reset stone counter
        floatingStones: [  // ⭐ Reset all stones to unclicked
          { id: 1, clicked: false, x: 20, y: 30 },  // Head stone
          { id: 2, clicked: false, x: 70, y: 20 },  // Trunk stone
          { id: 3, clicked: false, x: 30, y: 60 },  // Body stone
          { id: 4, clicked: false, x: 80, y: 50 }   // Legs stone
        ],
        ganeshaAnimation: 'breathing',  // ⭐ Reset animation
        ganeshaSize: 0.8,  // ⭐ Reset to small size
        ganeshaGlow: 0.2,  // ⭐ Reset glow
        currentFocus: 'growing'
      });
    }
  }}
  style={{
