...
useEffect(() => {
  ...
  // NEW: Handle fog area clicks (3-click sequence)
const handleFogClick = (emotionId) => {
  // Can only click if this crystal is active
  ...
  // Increment click count for this fog
  setFogProgress(prev => {
...
{!showPowerModal && !showRescueModal && !showCenteredSymbol && !showStoryModal && (
  <>
    ...
    {sceneState.phase === CAVE_PHASES.CRYSTAL_FOG_ACTIVE && sceneState.clearedFogs?.length < 3 && (
      <div className="phase-header">
    🔮 CLEAR THE FOG! Click a crystal to start!
      </div>
    )}
