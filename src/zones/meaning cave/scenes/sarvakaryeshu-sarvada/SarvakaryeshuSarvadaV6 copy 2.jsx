...
const handleSymbolClick = (symbolId, isWise) => {
  ...
  if (alreadyTried) {
    console.log('🚫 Symbol already tried:', symbolId);
    
    // Show quick notification
    sceneActions.updateState({
      characterMessage: "I already tried that... let me choose another! 💭"
    });
  ...
  if (isWise) {
    ...
    // ✅ WRONG CLICK: Add to tried list
    const newUnwiseTried = [...(sceneState.unwiseChoicesTried || []), symbolId];
    
    sceneActions.updateState({
      unwiseChoicesTried: newUnwiseTried, // ← Grey it out
      characterMessage: reaction,
      symbolsClickable: false
    });
    ...
    setTimeout(() => {
      sceneActions.updateState({
        characterMessage: null,
        symbolsClickable: true // Re-enable others
      });
      setShowSparkle(null);
    }, 2000);
  }
...
const handleDoor2Complete = () => {
  ...
  console.log(`✨ Helper clicked: ${helperId}`);
  
  // Keep symbols visible, just disable clicking
  sceneActions.updateState({
    symbolsClickable: false,
...
{sceneState.symbolsVisible === 'clear' && (() => {
  ...
  return symbols.map((symbol, index) => {
    ...
    const hasBeenTried = (sceneState.unwiseChoicesTried || []).includes(symbol.id); // ← Check if tried
    ...
    return (
      ...
      >
        <ClickableElement 
          ...
          disabled={!sceneState.symbolsClickable || hasBeenTried} // ← Disable if tried
        >
