...
import HelperSpinWheel from './components/HelperSpinWheel';
...
const SarvakaryeshuSarvada = ({ onComplete, onNavigate, zoneId = 'cave-of-secrets', sceneId = 'sarvakaryeshu-sarvada' }) => {
  ...
  phase: SCENE_PHASES.STORY_INTRO, // ← NEW phase
...
showHelperAnimation: false,    // ← ADD THIS
  helperScreenPosition: null, // ← ADD THIS
...
const SarvakaryeshuSarvadaContent = ({
  ...
  const [showGratitudeMessage, setShowGratitudeMessage] = useState(false);
const [gratitudeText, setGratitudeText] = useState('');

const [showMissionComplete, setShowMissionComplete] = useState(false);
const [completedMissionNumber, setCompletedMissionNumber] = useState(0);
...
const handleHelperAnimationComplete = () => {
  ...
  // Hide animation and transform background
  sceneActions.updateState({
  ...
  // ✨ Show gratitude message (selection card STAYS visible)
  setTimeout(() => {
    const currentScenario = getCurrentGame2Scenario();
    setGratitudeText(currentScenario.gratitude);
    setShowGratitudeMessage(true);
    
    // Hide gratitude after 2.5 seconds
    setTimeout(() => {
      setShowGratitudeMessage(false);
      
      // ✨ Show Mission Complete popup
      setTimeout(() => {
        const missionNumber = sceneState.game2ScenariosCompleted + 1;
        setCompletedMissionNumber(missionNumber);
        setShowMissionComplete(true);
        
        // Hide popup and complete scenario after 2 seconds
        setTimeout(() => {
          setShowMissionComplete(false);
          
          // Small delay then complete scenario
          setTimeout(() => {
            completeGame2Scenario();
          }, 500);
        }, 2000); // Mission complete popup shows for 2 seconds
      }, 300); // Small gap after gratitude
    }, 2500); // Gratitude shows for 2.5 seconds
  }, 500); // Small delay after transformation
};
...
const handleWheelHelperSelected = (selectedHelperData) => {
  console.log('🎡 Wheel selected:', selectedHelperData.name);
  console.log('📍 Helper position:', selectedHelperData.screenPosition);
  
  // Store helper position for animation
  sceneActions.updateState({
    selectedHelper: selectedHelperData.id,
    helperScreenPosition: selectedHelperData.screenPosition, // ← ADD THIS
    showHelperAnimation: true
  });
...
{/* Game 2: Helper Missions */}
{sceneState.currentGame === 2 && !sceneState.game2Completed && (
  <div className="game2-split-layout">
    
    {/* LEFT SIDE: Scenario Window */}
    <div className="game2-left-side">
      <div className="scenario-window-left">
        <img 
          src={getCurrentGame2Scenario().backgrounds[sceneState.game2Background]}
          alt="scenario"
        />
      </div>
      ...
      {sceneState.symbolsVisible === 'none' && (
        <div className="person-problem-speech-left">
          <p className="nunito-text">
            {getCurrentGame2Scenario().problemDeclaration}
          </p>
        </div>
      )}
    </div>
    ...
    <div className="game2-right-side">
      {sceneState.symbolsVisible === 'clear' && !sceneState.selectedHelper && (
        <HelperSpinWheel
          helpers={getCurrentGame2Scenario().helpers}
          onHelperSelected={handleWheelHelperSelected}
          disabled={sceneState.selectedHelper !== null}
        />
      )}
    </div>
...
{sceneState.showHelperAnimation && sceneState.selectedHelper && (
  <HelperSignatureAnimation
    ...
    fromPosition={sceneState.helperScreenPosition || { x: 75, y: 38 }}
    toPosition={{ x: 25, y: 40 }}
    onAnimationComplete={handleHelperAnimationComplete}
    onAnimationStart={(id, name) => console.log(`🎭 ${name} started`)}
  />
)}

{/* Gratitude Message - After Helper Animation */}
{showGratitudeMessage && (
  <div className="gratitude-speech-bubble">
    <div className="gratitude-heart">❤️</div>
    <p className="nunito-text">{gratitudeText}</p>
  </div>
)}
...
{showMissionComplete && (
  <div className="mission-complete-overlay">
    <div className="mission-complete-popup">
      <div className="mission-complete-star">⭐</div>
      <h2 className="baloo-heading mission-complete-title">
        Mission {completedMissionNumber} Complete!
      </h2>
      <p className="nunito-text mission-complete-text">
        You're an amazing helper! 🎉
      </p>
      <div className="mission-complete-badge">
        <span className="mission-badge-icon">❤️</span>
        <span className="mission-badge-text">Helper Badge Earned!</span>
      </div>
    </div>
  </div>
)}
