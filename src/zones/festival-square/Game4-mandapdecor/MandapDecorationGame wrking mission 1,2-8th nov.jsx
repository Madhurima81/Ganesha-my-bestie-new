...
const getMissionData = (selectedMission, currentStep, fixedCount) => {
  ...
  if (selectedMission.type === 'fix') {
    return {
      ...
      currentProgress: fixedCount,
      items: selectedMission.wrongPlacements,
      instruction: 'Tap red decorations to fix them',
      emoji: '🔧'
    };
  } else {
    // Puja Prep or other step-based missions
    return {
      type: 'steps',
      totalSteps: selectedMission.steps?.length || 5,
      currentProgress: currentStep,
      currentStepData: selectedMission.steps?.[currentStep - 1],
      items: selectedMission.steps,
      instruction: selectedMission.steps?.[currentStep - 1]?.instruction || '',
      emoji: selectedMission.steps?.[currentStep - 1]?.emoji || selectedMission.icon
    };
  }
};
  ...
  if (selectedMission?.type === 'fix' && selectedWrongItem) {
    ...
    if (newFixedCount >= 5) {
      safeSetTimeout(() => {
        ...
        //setUnlockedMissions(prev => ({ ...prev, 'eco-mandap': true }));
      ...
      <div className="mission-progress-minimal">
        ...
        <div className="progress-dots-minimal">
          {Array.from({ length: missionData.totalSteps }).map((_, i) => (
            <span 
              key={i} 
              className={`dot-mini ${i < (missionData.type === 'fix' ? fixedCount : completedSteps.length) ? 'filled' : ''}`} 
            />
          ))}
        </div>
      </div>
...
{Object.entries(MANDAP_ZONES).map(([zoneId, zone]) => {
  ...
  const isFixMode = missionData?.type === 'fix';
  ...
  // In Puja Prep: only show correct zone
  // In Fix mode: show all zones
  if (isInMission && !isFixMode && !isCorrectZone) {
    return null;
  }
