...
if (selectedMission && currentMode === GAME_MODES.CHALLENGE) {
  const currentStepData = selectedMission.steps[currentStep - 1];
  const placedItemId = decoration.id;
  
  // Check if correct item
  if (placedItemId !== currentStepData.item) {
  ...
  // ✅ CORRECT ITEM + CORRECT ZONE!
  
  // 1. Store step data BEFORE advancing
  setLastCompletedStep(currentStepData);  // ✅ ADD THIS LINE!
  
  // 2. Place the decoration
  ...
  // 5. Move to next step or complete mission
  if (currentStep < selectedMission.steps.length) {
    ...
    safeSetTimeout(() => {
      ...
      setUnlockedMissions(prev => ({ ...prev, 'fix-mandap': true }));
...
if (currentMode === GAME_MODES.FREE_PLAY || (currentMode === GAME_MODES.CHALLENGE && selectedMission && !showMissionIntro)) {
  ...
  const currentStepData = isInMission ? selectedMission.steps[currentStep - 1] : null;
...
{isInMission && currentStepData && (
  <div className="mission-minimal-header">
    <div className="mission-instruction-minimal">
      <span className="instruction-icon">{currentStepData.emoji}</span>
      <span className="instruction-text">
        {currentStepData.instruction.split('!')[0]}
      </span>
    </div>
    <div className="mission-progress-minimal">
      <span className="progress-count">{currentStep}/5</span>
      <div className="progress-dots-minimal">
        {Array.from({ length: 5 }).map((_, i) => (
          <span key={i} className={`dot-mini ${i < completedSteps.length ? 'filled' : ''}`} />
        ))}
      </div>
    </div>
  </div>
...
{/* Mission Progress Card */}
        {isInMission && (
          <div className="mission-progress-card">
            <h3 className="mission-progress-title">{selectedMission.icon} {selectedMission.name}</h3>
            <p className="mission-progress-subtitle">Step {currentStep} of {selectedMission.steps.length}</p>
            
            <ul className="mission-steps-list">
              {selectedMission.steps.map((step, index) => {
                ...
                return (
                  <li 
                    key={step.step}
                    className={`mission-step-item ${isCompleted ? 'completed' : ''} ${isActive ? 'active' : ''}`}
                  >
                    <span>{isCompleted ? '✅' : isActive ? '▶️' : '⭕'}</span>
                    <span>{stepNumber}. {step.emoji} {step.instruction.split('!')[0].split('.')[1] || 'Step'}</span>
                  </li>
                );
              })}
            </ul>
            
            <div className="mission-stars-display">
              ⭐ {completedSteps.length}/{selectedMission.steps.length}
            </div>
          </div>
        )}
...
// In zone rendering - SIMPLE VERSION
{Object.entries(MANDAP_ZONES).map(([zoneId, zone]) => {
  const isInMission = currentMode === GAME_MODES.CHALLENGE && selectedMission;
  const currentStepData = isInMission ? selectedMission.steps[currentStep - 1] : null;
  const isCorrectZone = isInMission && zoneId === currentStepData?.zone;
  const hasDecorations = gameState.placedDecorations.has(zoneId);
  const isHighlighted = highlightedZones.has(zoneId);
  
  // ✅ IN MISSION MODE: Only show/enable the correct zone
  if (isInMission && !isCorrectZone) {
    return null; // Don't render wrong zones at all!
  }
...
{isInMission ? (
  /* MISSION MODE: Show 5-step panel */
  ...
  /* FREE PLAY: Show category buttons */
      ...
      {gameState.selectedCategory && gameState.phase !== PHASES.COMPLETE && (
        ...
        >
          ...
          <div className="item-buttons">
           {DECORATION_CATEGORIES[gameState.selectedCategory.toUpperCase()]?.items
  .filter(item => {
    const isInMission = currentMode === GAME_MODES.CHALLENGE && selectedMission;
    const currentStepData = isInMission ? selectedMission.steps[currentStep - 1] : null;
    if (!isInMission) return true; // Free play - show all items
    return item.id === currentStepData?.item; // Mission - show ONLY needed item
  })
  .map((item) => (
              <div
                key={item.id}
