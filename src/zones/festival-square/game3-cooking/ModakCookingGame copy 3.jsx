...
<GamePauseMenu
  ...
  hasDesignOption={false}  // 👈 FALSE for Modak (no design selection)
...
onContinue={() => {
  console.log('🍪 MODAK CONTINUE: Going to next game + preserving resume');
  
  const profileId = localStorage.getItem('activeProfileId');
  if (profileId) {
    ProgressManager.updateSceneCompletion(profileId, 'festival-square', 'game3', {
      completed: true,
      stars: gameState.stars,
      badges: { cooking: true }
    });
    
    GameStateManager.saveGameState('festival-square', 'game3', {
      completed: true,
      stars: gameState.stars,
      badges: { cooking: true }
    });
    
    console.log('✅ MODAK CONTINUE: Completion data saved');
  }
  
  // Set next scene for resume
  setTimeout(() => {
    SimpleSceneManager.setCurrentScene('festival-square', 'game4', false, false);
    console.log('✅ MODAK CONTINUE: Next game (game4) set for resume tracking');
...
onReplay={() => {
  ...
  if (profileId) {
    // Clear ALL storage
    localStorage.removeItem(`temp_session_${profileId}_festival-square_game3`);
    localStorage.removeItem(`replay_session_${profileId}_festival-square_game3`);
    localStorage.removeItem(`play_again_${profileId}_festival-square_game3`);
    localStorage.removeItem('modakGame');
    
    SimpleSceneManager.setCurrentScene('festival-square', 'game3', false, false);
    console.log('🗑️ MODAK: All storage cleared');
  }
  ...
  console.log('🔄 MODAK: Game reset complete');
}}
