...
async function main() {
  ...
  const data = await page.evaluate(async () => {
    const waitForReady = () => new Promise((resolve, reject) => {
      ...
      const tick = () => {
        ...
        if (Date.now() - started > 45000) { reject(new Error("timeout")); return; }
        setTimeout(tick, 100);
      };
      tick();
    });
  ...
  fs.writeFileSync(path.resolve(process.cwd(), "tmp_t15_situations.json"), JSON.stringify(data, null, 2), "utf8");
