...
function collectContextWords(context) {
  return words([
    context.situation && context.situation.title,
...
function chooseSituation(libraries, request, issues) {
  ...
  let pool = situations.slice();
  if (request.needId) {
    pool = pool.filter((item) => item.hard && item.hard.need_id === request.needId);
  }
  if (request.lifeDomainId) {
    pool = pool.filter((item) => (item.hard && item.hard.life_domain_ids || []).includes(request.lifeDomainId));
  }
  if (!pool.length) {
    pool = situations.slice();
  }
  return sortByScore(pool.map((item) => ({ item, score: (item.hard && item.hard.severity) || 0 }))).map((entry) => entry.item)[0] || null;
}
...
function validatePlanner(context, libraries) {
  ...
  if (context.situation && context.need && context.situation.hard.need_id !== context.need.id) {
...
function resolvePhase6(libraries, request, overrides = {}) {
  ...
  const needId = request.needId || (situation.hard && situation.hard.need_id);
  ...
  const belief = {
    beliefIds: (situation.hard && situation.hard.belief_ids) || [],
    falseBelief: situation.false_belief_text,
    trueBelief: situation.true_belief_text,
  };
  ...
  const characterCandidates = topRanked(
    ...
    (item) => {
      let score = 10;
      if (request.lifeDomainId && (situation.hard.life_domain_ids || []).includes(request.lifeDomainId)) {
        score += 2;
      }
  ...
  const archetypeCandidates = topRanked(
    ...
    (item) => {
      ...
      score += textScore(item.description, words(`${situation.title} ${belief.trueBelief}`));
      return score;
    }
  );
  ...
  const missionTypeCandidates = topRanked(
    ...
    (item) => {
      let score = 5;
      score += item.id === situation.soft_suggested.ganesha_symbol_primary ? 0 : 0;
      return score;
    },
    5
  );
  ...
  const missionCandidates = topRanked(
    ...
    (item) => {
      ...
      score += countOverlap(item.bestLifeDomainIds || [], situation.hard.life_domain_ids || []) * 2;
      return score;
    }
  );
  ...
  const worldCandidates = topRanked(
    ...
    (item) => {
      ...
      score += textScore(item.name, words(situation.title));
      return score;
    }
  );
  ...
  const structureCandidates = topRanked(
    ...
    (item) => textScore(`${item.name} ${item.core_pattern} ${item.scene_flow_template}`, words(`${situation.title} ${belief.trueBelief}`))
  );
...
function getSituationCategories() {
  ...
  return unique((state.libraries.situations || []).map((item) => item.category).filter(Boolean)).sort((a, b) => a.localeCompare(b));
}
...
function populateCategorySelect() {
  ...
  const categories = getSituationCategories();
...
function categoryCardMeta(category) {
  const normalized = String(category || "").toLowerCase();
  if (normalized.includes("friend")) {
    return { icon: "🤝", label: "Friendship" };
  }
  if (normalized.includes("school")) {
    return { icon: "🏫", label: "School" };
  }
  if (normalized.includes("fear") || normalized.includes("insecurity")) {
    return { icon: "✨", label: "Worries & Courage" };
  }
  if (normalized.includes("anger") || normalized.includes("emotion") || normalized.includes("feeling")) {
    return { icon: "🙂", label: "Feelings & Emotions" };
  }
  if (normalized.includes("change") || normalized.includes("growing")) {
    return { icon: "🌱", label: "Growing Up" };
  }
  if (normalized.includes("family")) {
    return { icon: "👨‍👩‍👧", label: "Family" };
  }
  if (normalized.includes("moral") || normalized.includes("value")) {
    return { icon: "🎉", label: "Celebrations & Community" };
  }
...
function renderCategoryCards() {
  ...
  root.innerHTML = categories.map((category) => {
    ...
    return `
      <button class="category-card${select.value === category ? " active" : ""}" data-category-card="${escapeHtml(category)}" type="button">
...
function getSituationMatches(query, category) {
  if (!state.libraries) {
    return [];
  }
  const situations = (state.libraries.situations || []).filter((item) => !category || item.category === category);
  if (!query) {
  ...
  return sortByScore(
    situations.map((item) => ({
      item,
      score: textScore(`${item.title} ${item.category} ${item.false_belief_text} ${item.true_belief_text}`, queryWords),
    }))
  )
...
function populateSituationSelect(query, category = byId("categorySelect") && byId("categorySelect").value) {
  ...
  if (!matches.length) {
    byId("situationSelect").innerHTML = `<option value="">No situations found</option>`;
...
function pickSurpriseSituationId() {
  const pool = state.libraries.situations
    .slice()
    .sort((a, b) => (b.hard.severity || 0) - (a.hard.severity || 0));
...
function renderPreview(result) {
  ...
  byId("previewFlow").innerHTML = nodes
    .map(([label, value]) => `<div class="preview-node"><span>${escapeHtml(label)}</span>${escapeHtml(value || "")}</div>`)
    .join("");
}
...
function renderSeedSummary(result) {
  const ctx = result.context;
  setText("seedSummary", [
    ctx.situation && ctx.situation.title,
    ctx.need && titleCaseFromId(ctx.need.id),
    ctx.character && firstName(ctx.character.name),
  ].filter(Boolean).join(" | "));
  setHtml("seedExplain", [
    `<p><strong>Feeling:</strong> ${escapeHtml(ctx.belief && ctx.belief.falseBelief)}</p>`,
    `<p><strong>Growth:</strong> ${escapeHtml(ctx.belief && ctx.belief.trueBelief)}</p>`,
    `<p><strong>Hero path:</strong> ${escapeHtml(firstName(ctx.character && ctx.character.name))} learns through action, choice, and calm courage.</p>`,
  ].join(""));
}
...
function renderHeroCards() {
  ...
  const autoCard = `
    <button type="button" class="hero-card${currentId ? "" : " selected"}" data-character="">
      <strong>Best Match</strong>
      <div>Let the story choose the strongest fit.</div>
      <p>The engine picks the hero that best suits this situation and emotional shift.</p>
    </button>
  `;
  const heroCards = (state.libraries && state.libraries.characters || []).map((character) => `
    <button type="button" class="hero-card${String(character.id) === String(currentId) ? " selected" : ""}" data-character="${escapeHtml(character.id)}">
      <strong>${escapeHtml(firstName(character.name))}</strong>
      <div>${escapeHtml(character.hard && character.hard.roleId ? titleCaseFromId(character.hard.roleId) : "Story Hero")}</div>
      <p>${escapeHtml(character.core_personality || character.description || "A child hero ready for a meaningful adventure.")}</p>
    </button>
  `).join("");

  setHtml("heroCardGrid", autoCard + heroCards);
  byId("heroCardGrid").querySelectorAll("[data-character]").forEach((button) => {
    button.addEventListener("click", () => {
...
function renderStoryPreviewCard(result) {
  ...
  const situation = ctx.situation && ctx.situation.title || "a meaningful everyday moment";
  const missionName = ctx.mission && ctx.mission.name || "make things right";
  ...
  setText(
    "storyPreviewText",
    `${heroName} begins with ${situation.toLowerCase()}, faces a story about ${missionName.toLowerCase()}, and grows from "${ctx.belief && ctx.belief.falseBelief}" toward "${ctx.belief && ctx.belief.trueBelief}".`
  );
}
...
function renderReader() {
  ...
  setProductStep(5);
  byId("storyReader").classList.remove("hidden");
...
function openLibrary() {
  scrollToElement("savedStoriesSection");
}

function openActivities() {
  scrollToElement("activitiesSection");
}
...
function renderSavedStories() {
  if (!state.savedStories.length) {
    setHtml("savedStoriesGrid", `<div class="saved-card empty-library-card"><strong>Your story shelf is empty.</strong><div class="saved-meta">Create your first adventure.</div></div>`);
    updateSaveButtons();
    return;
  }
...
function buildCompleteStoryMaster(storyPlan, storyBlueprint, libraries) {
  ...
  const sceneTexts = (storyPlan.scenePlan || []).map((scene, index, scenes) => {
    ...
    if (isFirst) {
      return {
        sceneId: scene.id,
        text: `${protagonist} stepped into ${worldName} carrying the feeling of ${situationTitle.toLowerCase()}. When the first hard moment arrived, the old thought rushed up at once: "${falseBelief}" Nearby, a quiet shape of ${symbolLabel.toLowerCase()} rested in the world, but ${protagonist} was too upset to understand why it felt important.`,
      };
    }
...
function buildStoryBlueprint(result, request) {
  ...
  const requestLifeDomain = request.lifeDomainId || (ctx.situation && ctx.situation.hard.life_domain_ids && ctx.situation.hard.life_domain_ids[0]) || undefined;
...
function bindProductActions() {
  ...
  byId("anotherStoryBtn").addEventListener("click", rerollStoryPath);
  ...
  byId("homeCreateStoryBtn").addEventListener("click", () => scrollToElement("stepSituation"));
...
function syncVisibleSeedState(result) {
  ...
  if (state.seedMode !== "situation") {
    ...
    if (byId("categorySelect")) {
      byId("categorySelect").value = ctx.situation.category || byId("categorySelect").value;
    }
    populateSituationSelect("", ctx.situation.category);
...
async function loadLibraries() {
  const entries = await Promise.all(
    Object.entries(dataFiles).map(async ([key, url]) => [key, await loadJson(url)])
  );
