...
- [ ] Fun-fact / info VO → Already-placed slot tapped- removed
- [ ] Vo when all 4 placed 
...
### 19G · UX POLISH — RESPONSIVE DESIGN VERIFICATION
- [ ] **Touch target audit: All deity circles ≥ 70px diameter, tappable without zoom**
- [ ] **Touch target audit: All family avatar input zones ≥ 70px**
- [ ] **Choice modal buttons ≥ 70px tap height, no cramping on mobile**
- [ ] **Name input modal: text field ≥ 70px height for keyboard accessibility**
- [ ] **Responsive breakpoints: mobile (max-width 400px), tablet (400–600px), desktop (>600px)**
- [ ] @media (max-width: 600px) — deity circles scale proportionally, no overlap
- [ ] @media (max-width: 600px) — family card grid wraps cleanly, no horizontal scroll
- [ ] @media (max-width: 600px) — comparison card side-by-side layout stacks or scales down
- [ ] Font consistency: Baloo 2 for circle labels and headers, Nunito for descriptions
- [ ] Text contrast readable on all backgrounds (family_background.jpg)
- [ ] Correct placement: sparkle animation timing consistent across all circles
- [ ] Wrong choice: gentle wiggle feedback is not jarring on small screens
- [ ] Modal z-index: choice modal and name input modal render above tree overlay
- [ ] Z-index correct: sparkle > modal > tree > background
- [ ] SymbolSidebar does NOT appear in this scene
- [ ] AudioToggle button ≥ 70px (shared component, verify sizing in myindianstory)
...
### 20H · UX POLISH — RESPONSIVE DESIGN VERIFICATION
- [ ] **Touch target audit: All choice modal buttons ≥ 70px tap height**
- [ ] **Touch target audit: Friend input button (friend-intro-btn) ≥ 70px height**
- [ ] **Color choice buttons ≥ 70px height on mobile, no cramping**
- [ ] **Activity choice buttons ≥ 70px height on mobile, no cramping**
- [ ] **Responsive breakpoints: mobile (max-width 400px), tablet (400–600px), desktop (>600px)**
- [ ] @media (max-width: 600px) — choice modal cards scale proportionally (230px → 160px for mobile)
- [ ] @media (max-width: 600px) — 3-card Ganesha choice grid wraps cleanly, no horizontal scroll
- [ ] @media (max-width: 600px) — 7-card child food grid wraps cleanly, maintains touch target size
- [ ] @media (max-width: 600px) — color/activity/friend grids remain responsive, ≥ 70px buttons
- [ ] @media (max-width: 600px) — comparison card side-by-side layout stacks or scales proportionally
- [ ] Font consistency: Baloo 2 for phase titles, Nunito for instructions and labels
- [ ] Text contrast readable on all backgrounds (fav_background.jpg)
- [ ] Correct choice: glow + sparkle feedback satisfying and immediate on all screen sizes
- [ ] Wrong choice: gentle wiggle feedback is not jarring on mobile
- [ ] Modal z-index: choice modal and drawing/text input modals render above cards
- [ ] Z-index correct: sparkle > modal > grid > background
- [ ] SymbolSidebar does NOT appear in this scene
- [ ] AudioToggle button ≥ 70px (shared component, verify sizing in Favoritefoodgame)
...
### 21H · UX POLISH — RESPONSIVE DESIGN VERIFICATION
- [ ] **Touch target audit: wish1 bubbles ≥ 70px tap radius, no accidental misses**
- [ ] **Touch target audit: wish2 food items ≥ 70px, draggable without cramping**
- [ ] **Touch target audit: wish3 forest spots ≥ 70px tap zones, clearly visible**
- [ ] **Touch target audit: dream-clouded trunk tap zone ≥ 70px, tappable**
- [ ] **Responsive breakpoints: mobile (max-width 400px), tablet (400–600px), desktop (>600px)**
- [ ] @media (max-width: 600px) — wish1 bubble container scales, maintains spacing between bubbles
- [ ] @media (max-width: 600px) — wish2 food items scale proportionally, drop zones remain ≥ 70px
- [ ] @media (max-width: 600px) — wish3 forest background and spot zones scale without distortion
- [ ] @media (max-width: 600px) — dream canvas drawing area remains responsive, no toolbar overflow
- [ ] @media (max-width: 600px) — dream-clouded progress visual (cloud fade) clear at small sizes
- [ ] Font consistency: Baloo 2 for phase titles, Nunito for wish descriptions and nudges
- [ ] Text contrast readable on all backgrounds (dream_background.jpg)
- [ ] Bubble tap feedback: gentle shake on unkind bubble is not jarring on small screens
- [ ] Kind bubble sparkle animation satisfying and immediate on all devices
- [ ] Drag/drop feedback: food items respond smoothly on mobile without lag
- [ ] Spot reveal sparkle effect visible and celebratory on all screen sizes
- [ ] Cloud fade animation smooth on all devices, no jank on mobile
- [ ] Modal z-index: drawing modal and any tooltips render above dream scene
- [ ] Z-index correct: sparkle > modal > scene > background
- [ ] SymbolSidebar does NOT appear in this scene
- [ ] AudioToggle button ≥ 70px (shared component, verify sizing in ObstacleRemoverGame)
